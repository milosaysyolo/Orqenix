...
function fixContent(content, filePath) {
  ...
  const codeRegex = /\b(import\s+(type\s+)?[\{*]|export\s+(type\s+)?[\{*]|export\s+(default\s+)?(class|function|interface|type|const|enum)|abstract\s+class|class\s+\w+\s*(extends|implements|\{)|function\s+\w+\s*\(|\/\*\*)/;
