...
const CODE_STARTERS = /\b(import\s|export\s|\/\*\*|abstract\s|class\s|function\s|const\s|let\s|var\s|type\s|interface\s|enum\s|async\s)/;

function fixContent(content, filePath) {
  if (!content.startsWith(SPDX)) return content;
  const lines = content.split('\n');
  if (lines.some(l => l.trim() && !l.trim().startsWith('//') && !l.trim().startsWith('*'))) return content;
  ...
  for (let pi = 0; pi < parts.length; pi++) {
    ...
    if (parts[pi] === '//' && pi + 1 < parts.length) {
      ...
      if (codeMatch && codeMatch.index > 0) {
        ...
        const commentPart = commentContent.slice(0, codeMatch.index).replace(/[^a-zA-Z0-9)\]]$/, '');
...
for (const f of files) {
  ...
  try {
    committedContent = execSync(`git show HEAD:"${f}"`, { encoding: 'utf-8', stdio: 'pipe' }).trim();
  } catch {
