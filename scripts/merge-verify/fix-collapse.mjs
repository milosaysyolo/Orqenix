...
async function fix(relPath, fixer) {
  ...
  if (before !== after) {
    ...
    console.log(`  FIXED: ${relPath} (${before.split('\n').length} -> ${after.split('\n').length} lines)`);
  } else {
...
function splitCodeSection(code) {
  const kws = ['import ','export ','const ','let ','var ','function ','class ','describe(','describe.(',
    'it(','it.(', 'test(','test.(', 'expect(','expect.(',
    'beforeAll(','afterAll(','beforeEach(','afterEach(',
    'return ','if (','if(','else ','for (','for(','while (','while(',
    'switch (','switch(','try {','try{','catch (','catch(','finally {','finally{',
    'throw ','async ','await ','new '];
  let r = code;
  for (const kw of kws) {
    r = r.split(';'+kw).join(';\n'+kw);
  }
  const kwPattern = 'import |export |const |let |var |function |describe\\(|it\\(|test\\(|expect\\(|beforeAll\\(|afterAll\\(';
...
await (async () => {
  try {
    const pkg = JSON.parse(await readFile(resolve(ROOT, 'packages/marketplace-ui/package.json'), 'utf-8'));
    if (pkg.scripts?.build) {
      await fix('packages/marketplace-ui/tsup.config.ts', () =>
