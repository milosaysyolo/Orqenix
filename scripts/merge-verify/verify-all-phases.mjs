...
results.push(
  run("Merge conflict scan", "node", ["scripts/merge-verify/scan-merge-conflicts.mjs"], {
    fatal: true,
  }),
);
...
results.push(
  run(
    "Migration ordering",
    "pnpm",
    ["vitest", "run", "tests/merge-verify/migration-ordering.test.ts"],
    { fatal: true },
  ),
);
...
results.push(
  run("Cross-phase seams", "pnpm", [
    "vitest",
    "run",
    "tests/merge-verify/cross-phase-integration.test.ts",
  ]),
);
results.push(
  run("Settings bootstrap flat keys", "pnpm", [
    "vitest",
    "run",
    "tests/merge-verify/settings-bootstrap-flat.test.ts",
  ]),
);
