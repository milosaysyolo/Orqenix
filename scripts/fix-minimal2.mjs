...
function fix(file) {
  ...
  if (codeParts.length) {
    ...
    code = code.replace(
      /(\/\/[^\n]*?)\s{4,}(return |const |let |var |if |for |while |switch |try |catch |finally |throw |this\.|await |async |function |class )/g,
      "$1\n$2",
    );
    ...
    code = code.replace(
      /;(\s*)(export |import |abstract |class |function |const |let |var |type |interface |enum |async |public |private |protected |static |readonly )/g,
      ";\n$2",
    );
