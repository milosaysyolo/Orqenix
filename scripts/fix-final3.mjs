...
function extractCodeAfter(line) {
  ...
  const codeKw =
    /\b(import\s+|export\s+|\/\*\*|abstract\s+class|class\s+\w+|function\s+\w+|const\s+\w+\s*[:=]|let\s+\w+|var\s+\w+|type\s+\w+\s*=|interface\s+\w+|enum\s+\w+)/;
  const m = line.match(codeKw);
  if (m && m.index > 0) {
    return {
      comment: line.slice(0, m.index).replace(/[^a-zA-Z0-9)\]}\/]+$/, ""),
      code: line.slice(m.index),
    };
  }
  return null;
}
...
function fixContent(content, filePath) {
  ...
  if (codeContent.trim()) {
    ...
    code = code.replace(
      /}([^}]+?)(export|import|abstract|class|function|const|let|var|type|interface|enum|async|public|private|protected|static|readonly)/g,
      "}\n$2",
    );
    ...
    code = code.replace(
      /;(\s*)(export|import|abstract|class|function|const|let|var|type|interface|enum|async|public|private|protected|static)/g,
      ";\n$2",
    );
