...
function fix(file) {
  ...
  const codeStart =
    /\b(import\s+|export\s+|\/\*\*|abstract\s+class|class\s+\w+[\s\{]|function\s+\w+)/;
  ...
  if (codeRaw) {
    ...
    const inlineKw =
      /\b(return|const|let|var|if|for|while|switch|try|catch|finally|throw|this\.|await|async|function|class)\b/;
    ...
    code = code.replace(/\/\/[^\n]*/g, (match) => {
      ...
      if (altMatch) {
        return (
          match.slice(0, match.indexOf(altMatch[1]) + 1) +
          "\n" +
