...
const claims = [
  ...
  { file: 'packages/ui-primitives/package.json', expectedField: 'devDependencies.tailwindcss-animate' },
];
...
for (const claim of claims) {
  try {
    ...
    if (!mainHas && !currentHas) {
      ...
      console.log(`\u274C ${claim.file}: ${claim.expectedField} REMOVED in current branch (regression)`);
      violations += 1;
    } else if (!mainHas && currentHas) {
      console.log(`\u2713 ${claim.file}: ${claim.expectedField} ADDED in current branch (intentional)`);
    } else {
