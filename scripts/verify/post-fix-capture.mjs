...
for (const target of targets) {
  ...
  const bindingError = /Could not locate the bindings file|NODE_MODULE_VERSION|cannot find module 'better-sqlite3'/i.test(stdout + stderr);
  ...
  console.log(`  binding error detected: ${bindingError ? '❌ YES (fix did NOT work)' : '✅ NO (fix worked!)'}`);
}
