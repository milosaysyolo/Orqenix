...
const ITEMS = [
  {
    num: 1, id: 'rerender-d7.4', name: 'Re-render D7.4 report', severity: 'P1',
    script: '01-rerender-d7.4-report.mjs', linuxOnly: false,
    evidenceKeywords: ['lines', 'PASS', 'clean'],
  },
  {
    num: 2, id: 'benchmarks', name: 'Measure real benchmarks', severity: 'P0',
    script: '02-measure-benchmarks.mjs', linuxOnly: false,
    evidenceKeywords: ['p95', 'env/s', 'MEASURED'],
  },
  {
    num: 3, id: 'fresh-clone', name: 'Fresh-clone smoke test', severity: 'P0',
    script: '03-fresh-clone-smoke.mjs', linuxOnly: false,
    evidenceKeywords: ['exports', 'symbol', 'OK'],
  },
  {
    num: 4, id: 'miniflare', name: 'Miniflare conformance', severity: 'P0',
    script: '04-miniflare-conformance.mjs', linuxOnly: true,
    evidenceKeywords: ['miniflare', 'binding', 'PASS'],
  },
  {
    num: 5, id: 'otlp-grpc', name: 'OTLP gRPC interop', severity: 'P0',
    script: '05-otlp-grpc-interop.mjs', linuxOnly: true,
    evidenceKeywords: ['orqenix.test.counter', 'received', 'collector'],
  },
  {
    num: 6, id: 'provenance', name: 'Provenance + cosign + tags', severity: 'P0',
    script: '06-verify-provenance-cosign.mjs', linuxOnly: false,
    evidenceKeywords: ['attestations', 'v0.7.0', 'OK'],
  },
  {
    num: 7, id: 'pro-deps', name: 'Pro deps republish audit', severity: 'P1',
    script: '07-pro-deps-republish-audit.mjs', linuxOnly: false,
...
function emitResult(item, status, durMs, reason = '') {
  const line = JSON.stringify({
    id: item.id, name: item.name, status, severity: item.severity,
    duration_ms: durMs, evidence: `evidence/item-${item.num}`,
...
log(`Platform: ${platform()}${IS_WINDOWS ? ' (linux-only items will SKIP unless --allow-windows)' : ''}`);
...
spawnSync('node', [resolve(__dirname, 'generate-report.mjs'),
  '--in', RESULTS, '--out', REPORT], { stdio: 'inherit' });
...
console.log(`  Verdict: ${verdict === 'GO' ? c.green(verdict) :
                          verdict === 'NO-GO' ? c.red(verdict) :
                          c.yellow(verdict)}`);
console.log(`  P0: ${p0Passes.length} PASS, ${p0Fails.length} FAIL, ${p0Skips.length} SKIP (of ${p0Total})`);
