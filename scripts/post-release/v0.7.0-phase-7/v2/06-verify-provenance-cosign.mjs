...
const OSS_PACKAGES = [
  '@orqenix-cloud/relay-protocol', '@orqenix-cloud/relay-transport',
  '@orqenix-cloud/relay-presence', '@orqenix-cloud/sdk',
  '@orqenix-cloud/billing-design', '@orqenix-cloud/phase6-to-phase7',
  '@orqenix-cloud/tenancy', '@orqenix-cloud/audit-core',
  '@orqenix-cloud/observability-otlp', '@orqenix-cloud/cloud-adapter-cloudflare',
  '@orqenix/cli',
];
const IMAGES = [
...
for (const pkg of OSS_PACKAGES) {
  const r = spawnSync('npm', ['view', `${pkg}@${VERSION}`, '--json'], { encoding: 'utf8', timeout: 30000 });
  const out = r.stdout || '{}';
...
try { execSync('cosign version', { stdio: 'pipe' }); cosignAvailable = true; } catch {}

if (cosignAvailable) {
  console.log(`[06] Verifying cosign signatures on ${IMAGES.length} images...`);
  for (const img of IMAGES) {
    const r = spawnSync('cosign', ['verify', img,
      '--certificate-identity-regexp', 'https://github.com/milosaysyolo/.*',
      '--certificate-oidc-issuer', 'https://token.actions.githubusercontent.com'],
      { encoding: 'utf8', timeout: 60000 });
    writeFileSync(resolve(EVIDENCE, `cosign-${img.replace(/[/:]/g, '_')}.log`),
      `${r.stdout || ''}\n${r.stderr || ''}`);
    if (r.status === 0) {
  ...
  console.log('[06] cosign not installed — image signature verification skipped (does NOT count as PASS)');
}

console.log(`[06] Verifying git tags on ${REPOS.length} repos...`);
for (const repo of REPOS) {
  const r = spawnSync('git', ['ls-remote', '--tags', `https://github.com/${repo}.git`, `v${VERSION}`],
...
writeFileSync(resolve(EVIDENCE, 'summary.json'), JSON.stringify({
