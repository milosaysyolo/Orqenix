...
const install = spawnSync('pnpm', ['--filter', '@orqenix-cloud/cloud-adapter-cloudflare',
  'add', '-D', 'miniflare@^3', 'wrangler@^3'], { encoding: 'utf8', cwd: process.cwd(), timeout: 5 * 60 * 1000 });
...
const conf = spawnSync('pnpm', ['--filter', '@orqenix-cloud/cloud-adapter-cloudflare',
  'exec', 'vitest', 'tests/miniflare-conformance.spec.ts', '--run'],
...
const bindings = ['KV', 'R2', 'DurableObject', 'Service', 'Queue']
  .filter(b => new RegExp(`${b}.*binding|binding.*${b}`, 'i').test(log));
