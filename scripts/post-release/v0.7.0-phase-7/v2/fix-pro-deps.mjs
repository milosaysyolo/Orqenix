...
const dirs = readdirSync(pkgsDir).filter(d =>
  existsSync(resolve(pkgsDir, d, 'package.json')));
...
if (!DRY && fixed > 0) {
  ...
  console.log('[fix-pro-deps] Next: cd ../Orqenix-Pro && pnpm install --no-frozen-lockfile && pnpm -r build && pnpm -r test');
}
