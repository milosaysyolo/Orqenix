...
let verdict, icon;
if (p0Fail.length > 0) { verdict = 'NO-GO'; icon = '❌'; }
else if (p0Skip.length > 0 || p0Pass.length < p0.length) { verdict = 'INCONCLUSIVE'; icon = '🟡'; }
else { verdict = 'GO'; icon = '✅'; }
...
items.forEach((it, i) => {
  ...
  lines.push(`| ${i + 1} | ${it.name} | ${it.severity} | ${s} ${it.status} | ${it.duration_ms}ms | ${it.reason || '-'} | \`${it.evidence}\` |`);
});
...
if (verdict === 'GO') {
  ...
  lines.push('2. If Pro deps issue: `node scripts/post-release/v0.7.0-phase-7/v2/fix-pro-deps.mjs`');
