...
catch { console.log('[05] docker not available — SKIP'); process.exit(2); }
...
const start = spawnSync('docker', ['run', '-d', '--rm', '--name', CONTAINER,
  '-p', `${PORT}:${PORT}`,
  '-v', `${configPath}:/etc/otelcol-contrib/config.yaml:ro`,
  IMAGE, '--config=/etc/otelcol-contrib/config.yaml'], { encoding: 'utf8' });
writeFileSync(resolve(EVIDENCE, 'collector-start.log'), `${start.stdout || ''}\n${start.stderr || ''}`);
...
const cleanup = () => { try { execSync(`docker stop ${CONTAINER}`, { stdio: 'pipe' }); } catch {} };
...
const test = spawnSync('node', [testFile], { encoding: 'utf8', cwd: process.cwd(), timeout: 30000 });
