...
function emit(id, name, status, severity, ms, ev) {
  appendFileSync(RESULTS, JSON.stringify({ id, name, status, severity, duration_ms: ms, evidence: ev, ts: new Date().toISOString() }) + '\n');
}
...
runItem({num:1,id:'rerender-d7.4',name:'Re-render D7.4 report',sev:'P1'}, () => {
  const src = resolve(ROOT, '../Universal-Multi-Agent-System-for-OpenCode/docs/Delivery/Report/D7.4-Implementation-Report.md');
...
runItem({num:2,id:'benchmarks',name:'Measure real benchmarks',sev:'P0'}, () => {
  const ev = resolve(EVIDENCE, 'item-2'); mkdirSync(ev, { recursive: true });
  tryRun('pnpm --filter @orqenix-cloud/relay-core build', ROOT);
  tryRun('pnpm --filter @orqenix-cloud/relay-transport build', ROOT);
  const s = { rtt_same_region_p95_ms:'MEASUREMENT_PENDING', rtt_cross_region_p95_ms:'MEASUREMENT_PENDING', throughput_env_per_sec:'MEASUREMENT_PENDING', targets:{ rtt_same_region_p95_ms:80, rtt_cross_region_p95_ms:250, throughput_env_per_sec:2000 }, note:'Offline; cross-region requires cloud deployment' };
...
runItem({num:3,id:'fresh-clone',name:'Fresh-clone smoke test',sev:'P0'}, () => {
  const ev = resolve(EVIDENCE, 'item-3'); mkdirSync(ev, { recursive: true });
  const pkgs = ['@orqenix-cloud/relay-protocol', '@orqenix-cloud/relay-transport', '@orqenix-cloud/billing-design'];
  for (const p of pkgs) {
...
runItem({num:5,id:'otlp-grpc',name:'OTLP gRPC interop',sev:'P0'}, () => {
  ...
  const r = tryRun('pnpm --filter @orqenix-cloud/observability-otlp exec vitest tests/grpc-native.test.ts --run', ROOT);
...
runItem({num:6,id:'provenance',name:'Verify provenance + cosign',sev:'P0'}, () => {
  const ev = resolve(EVIDENCE, 'item-6'); mkdirSync(ev, { recursive: true });
  const ossDir = resolve(ROOT, '../Orqenix');
  const proDir = resolve(ROOT, '../Orqenix-Pro');
  let fails = 0;
  if (!existsSync(ossDir)) { console.log('[06] OSS repo MISSING'); fails++; }
  else console.log(`[06] OSS repo OK: ${ossDir}`);
  if (!existsSync(proDir)) { console.log('[06] Pro repo MISSING'); fails++; }
  ...
  } catch(e) { console.log(`[06] Tag check: ${e.message}`); }
...
function runItemAsync(item, runFn) {
  ...
  return Promise.resolve(runFn()).then(result => {
    ...
    if (result.code === 0) {
      console.log(`[PASS] Item ${item.num}: ${item.name} (${ms}ms)`);
      emit(item.id, item.name, 'PASS', item.sev, ms, ePath); return 'pass';
    }
    if (result.code === 2) {
      console.log(`[SKIP] Item ${item.num}: ${item.name} (${ms}ms)`);
      emit(item.id, item.name, 'SKIP', item.sev, ms, ePath); return 'skip';
    }
...
runItemAsync({num:7,id:'pro-deps',name:'Pro deps republish audit',sev:'P1'}, () => {
  const ev = resolve(EVIDENCE, 'item-7'); mkdirSync(ev, { recursive: true });
  const proDir = resolve(ROOT, '../Orqenix-Pro');
  ...
  } catch(e) { return { code: 2, msg: `Cannot read Pro packages: ${e.message}` }; }
  let bad = 0;
  for (const f of pkgFiles) {
    try {
      ...
      for (const [k, v] of Object.entries(deps)) {
        if (String(v).startsWith('workspace:') || String(v).startsWith('file:') || String(v).startsWith('link:')) {
          hasBad = true;
          console.log(`  ${meta.name} -> ${k}@${v}`);
        }
      }
      if (hasBad) { bad++; console.log(`[07] FAIL ${meta.name}: unresolved local refs`); }
      else console.log(`[07] OK ${meta.name}: ${Object.keys(deps).length} deps clean`);
    } catch(e) { console.log(`[07] ERR ${f}: ${e.message}`); }
  }
  writeFileSync(resolve(ev, 'summary.json'), JSON.stringify({ checked: pkgFiles.length, failed: bad, note: bad ? 'Need republish' : 'All clean' }, null, 2));
  if (bad) return { code: 1, msg: `${bad} package(s) need republish` };
  return { code: 0 };
}).then(() => {
  // Finalize
  console.log('\n[ORCH] Generating report...');
  const data = readFileSync(RESULTS, 'utf8').trim().split('\n').filter(Boolean).map(l => JSON.parse(l));
  ...
  if (p0Fails.length > 0) { console.log('\n[FAIL] GO/NO-GO: NO-GO'); process.exit(1); }
  console.log('\n[PASS] GO/NO-GO: GO'); process.exit(0);
});
