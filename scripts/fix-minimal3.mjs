...
function fix(file) {
  ...
  if (codeParts.length) {
    ...
    const afterSemicolon = /;(\s*)(export |import |abstract |class |function |const |let |var |type |interface |enum |case\s|async |public |private |protected |static |readonly |\/\*\*)/g;
    ...
    code = code.replace(/(\/\/[^\n]*?\w)(\s*)(\})(\s*)(case\s|export\s|import\s|class\s|function\s|const\s|let\s|var\s|type\s|interface\s|enum\s|async\s)/g, '$1\n$3$4$5');
