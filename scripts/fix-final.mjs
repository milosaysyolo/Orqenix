...
function fixSingleLineCode(codeSection) {
  ...
  result = result.replace(/}\s*(export|import|abstract|class|function|const|let|var|type|interface|enum)/g, '}\n\n$1');
...
function fixContent(content, filePath) {
  ...
  const codeStartMatch = rest.match(/\b(import\s+(type\s+)?[\{*]|export\s+|\/\*\*|(abstract\s+)?class\s+\w+|function\s+\w+)/);
  let headerPart = rest;
  let codePart = '';
...
for (const f of files) {
  let content;
  try {
    content = execSync(`git show HEAD:"${f.replace(/\\/g, '/')}"`, { encoding: 'utf-8', stdio: 'pipe' }).trim();
  } catch {
