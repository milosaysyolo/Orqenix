...
class G15 extends GateRunner {
  ...
  protected loadSpec(): unknown {
    return readFileSync(join(REPO_ROOT, ".orqenix/charter-gates/G15.yaml"), "utf-8");
  }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check("G15.1", "compress-strategies unit tests pass", () => {
        execSync("npx vitest run", {
          cwd: join(REPO_ROOT, "packages/compress-strategies"),
          stdio: "pipe",
        });
      }),
      ...
      await this.check("G15.2", "smart-compression unit tests pass", () => {
        execSync("npx vitest run", {
          cwd: join(REPO_ROOT, "packages/smart-compression"),
          stdio: "pipe",
        });
      }),
      ...
      await this.check("G15.4", "drop preserves tier 0 under aggressive budget", async () => {
        ...
        if (!out.conversation.messages.find((x) => x.id === "m0"))
          throw new Error("tier 0 was dropped");
      }),
        ...
        const strat = new DistillStrategy({
          extract: (text, sourceId) =>
            text.includes("prefer")
              ? [{ type: "preference", content: text, confidence: 0.9, sourceMessageId: sourceId }]
              : [],
          memoryWriter: async (d) => {
            drafts.push(...d);
          },
        });
        const conv: Conversation = {
      ...
      await this.check("G15.7", "selectStrategy auto routes correctly", () => {
        const cfg = {
          targetTokens: 100,
          maxTokens: 300,
          overflowCapPercent: 105 as const,
          selectionPolicy: "auto" as const,
          defaultStrategy: "compress-chain" as const,
        ...
        const big: Conversation = {
          scopeId: SCOPE,
          messages: [m("m1", "user", "x".repeat(2000), 3, 0)],
        };
        if (selectStrategy(big, cfg).strategyId !== "compress-chain")
          throw new Error("big not chain");
      }),

      await this.check("G15.8", "engine raises on tier-0 forced violation / overflow", async () => {
        const conv: Conversation = {
          scopeId: SCOPE,
          messages: [m("m0", "system", "x".repeat(400), 0, 0)],
        };
        const engine = new SmartCompressionEngine({
          config: {
            targetTokens: 1,
            maxTokens: 50,
            overflowCapPercent: 105,
            selectionPolicy: "fixed",
            defaultStrategy: "drop",
          },
          strategies: { drop: new DropStrategy() },
          scopeId: SCOPE,
        });
        let caught = false;
        try {
          await engine.compress(conv);
        } catch (e: unknown) {
          caught = true;
