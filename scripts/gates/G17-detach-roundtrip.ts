...
class G17 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G17.yaml'), 'utf-8'); }
    ...
    return [
      ...
      await this.check('G17.2', 'planUnlink + execute revokes both directions', async () => {
        ...
        try {
          ...
          if (linkStore.get(B, 'outbound').status !== 'revoked') throw new Error('outbound not revoked');
          if (linkStore.get(B, 'inbound').status !== 'revoked') throw new Error('inbound not revoked');
      ...
      await this.check('G17.3', 'invalid confirmation token rejected', async () => {
        ...
        try {
          ...
          if (linkStore.get(B, 'outbound').status !== 'active') throw new Error('link mutated despite bad token');
        } finally { await tear(dir, conn); }
      }),
      await this.check('G17.4', 'dry run does not mutate links but writes audit entry', async () => {
        ...
        try {
          ...
          if (linkStore.get(B, 'outbound').status !== 'active') throw new Error('dry run mutated link');
          if (auditStore.count() !== before + 1) throw new Error('dry run did not record audit entry');
        } finally { await tear(dir, conn); }
      }),
      await this.check('G17.5', 'detach event chained into audit log + verifyChain green', async () => {
        const { dir, conn, linkStore, auditStore, planner, executor } = await setup();
        try {
          linkStore.create({ remoteScopeId: B, direction: 'outbound' });
          linkStore.updateStatus(B, 'outbound', 'active');
          ...
          if (auditStore.verifyChain().entriesChecked !== 3) throw new Error('audit chain wrong length');
        } finally { await tear(dir, conn); }
      }),
...
async function main(): Promise<void> {
  const r = new G17(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G17 crashed:', e); process.exit(2); });
