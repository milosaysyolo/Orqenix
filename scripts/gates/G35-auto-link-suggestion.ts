...
import { MeshRouter, InMemoryMeshTransport, type MeshQueryHit, type MeshQueryResponse } from '@orqenix/mesh-routing';
...
function hit(sid: string, t: string, s: number): MeshQueryHit { return { scopeId: sid, text: t, score: s, provenance: prov(sid) }; }
...
async function setup() {
  ...
  return { dir, conn, router: new MeshRouter({ localScopeId: A, linkStore, transport: new InMemoryMeshTransport() }) };
}
...
function failureResponse(sid: string): MeshQueryResponse {
  return {
    ...
    hits: [],
    outcomes: [{ scopeId: sid, ok: false, reason: 'timeout', message: 't', durationMs: 100 } as any],
...
function successResponse(sid: string, score: number): MeshQueryResponse {
  return {
    query: { text: 'q', k: 5, timeoutMs: 1000 } as any,
    scopesQueried: 1, scopesSucceeded: 1,
...
class G35 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G35.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G35.1', 'frequent-failure suggestion at >=60% failure rate over >=5 runs', async () => {
          ...
          for (let i = 0; i < 4; i++) {
            history.push({
              ...
              outcomes: [
                ...
                { scopeId: C, ok: true, hits: [hit(C, 'low', 0.20)], durationMs: 10 } as any,
              ],
              totalDurationMs: 10, quorumReached: true,
            });
          }
      ...
      await this.check('G35.4', 'empty history returns empty suggestions', async () => {
        const { dir, conn, router } = await setup();
        try {
          if (router.suggestLinks([]).length !== 0) throw new Error('non-empty result for empty history');
        } finally { await tear(dir, conn); }
      }),
      await this.check('G35.5', 'high-failure scope is NOT suggested as high-relevance', async () => {
        const { dir, conn, router } = await setup();
        try {
          const history: MeshQueryResponse[] = [];
          for (let i = 0; i < 5; i++) history.push(failureResponse(B));
          ...
          if (failureFound && relevanceFound) throw new Error('conflicting recommendations for same scope');
        } finally { await tear(dir, conn); }
      }),
    ];
  }
...
async function main(): Promise<void> {
  const r = new G35(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G35 crashed:', e); process.exit(2); });
