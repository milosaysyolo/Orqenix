...
function payload(seed: number): TokenPayload {
  return {
    ...
    iat: 1000 + seed, nbf: 1000 + seed, exp: 2000 + seed,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G27.1', 'unit tests pass', () => {
        execSync('pnpm --filter @orqenix/capability-tokens test', { cwd: REPO_ROOT, stdio: 'pipe' });
      }),
      await this.check('G27.2', '500 base64url round-trips', () => {
        for (let i = 0; i < 500; i++) {
          ...
          for (let j = 0; j < len; j++) if (back[j] !== bytes[j]) throw new Error(`byte mismatch iter ${i}`);
        }
      }),
...
main().catch((e) => { console.error('G27 crashed:', e); process.exit(2); });
