    ...
    return [
      ...
      await this.check('G3.3', 'expected tables and indexes exist', async () => {
        ...
        try {
          ...
          const tables = conn.prepare<{ name: string }>(
            `SELECT name FROM sqlite_master WHERE type='table'`,
          ).all() as Array<{ name: string }>;
          const names = tables.map((t) => t.name);
          for (const required of ['chat_sessions', 'chat_entries', 'chat_embeddings', '_orqenix_migrations']) {
      ...
      await this.check('G3.4', 'STRICT mode enforced on chat tables', async () => {
        ...
        try {
          const conn = new SqliteConnection({ path: join(dir, 'a.sqlite') });
          runMigrations(conn, CHAT_KB_MIGRATIONS);
          const ddl = conn.prepare<{ sql: string }>(
      ...
      await this.check('G3.5', 'checksum drift detected', async () => {
        ...
        try {
          const conn = new SqliteConnection({ path: join(dir, 'a.sqlite') });
          runMigrations(conn, CHAT_KB_MIGRATIONS);
          const drifted = [{ ...CHAT_KB_MIGRATIONS[0], sql: CHAT_KB_MIGRATIONS[0].sql + '\n-- tampered' }];
          let caught = false;
          try { runMigrations(conn, drifted); } catch { caught = true; }
...
main().catch((e) => { console.error('G3 crashed:', e); process.exit(2); });
