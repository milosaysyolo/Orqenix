...
import { WorkspaceStore, WORKSPACE_MIGRATIONS, OwnerRemovalError, MembershipAlreadyExistsError } from '@orqenix/workspace';
...
class G31 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G31.yaml'), 'utf-8'); }
    ...
    return [
      ...
      await this.check('G31.3', 'addMember rejects duplicates', async () => {
        ...
        try {
          ...
          let caught = false;
          try { store.addMember(w.id as any, B, 'observer'); } catch (e) { caught = e instanceof MembershipAlreadyExistsError; }
      ...
      await this.check('G31.4', 'removeMember refuses owner', async () => {
        ...
        try {
          const w = store.create({ name: 'team', ownerScopeId: A });
          let caught = false;
          try { store.removeMember(w.id as any, A); } catch (e) { caught = e instanceof OwnerRemovalError; }
      ...
      await this.check('G31.5', 'transferOwnership swaps roles atomically', async () => {
        ...
        try {
          ...
          if (store.getMember(w.id as any, A).role !== 'contributor') throw new Error('previous owner not demoted');
        } finally { await tear(dir, conn); }
      }),
...
async function main(): Promise<void> {
  const r = new G31(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G31 crashed:', e); process.exit(2); });
