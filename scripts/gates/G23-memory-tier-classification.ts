...
import {
  MemoryTierStore, MEMORY_TIER_MIGRATIONS,
  classifyInitialTier, evaluatePromotion, canDemote, nextTier,
  DEFAULT_POLICY, ImmutableMemoryError,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G23.1', 'memory-tiers unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/memory-tiers'), stdio: 'pipe' });
      }),
      ...
      await this.check('G23.4', 'promotion path working -> episodic -> semantic -> procedural', () => {
        const base = {
        ...
        const w = evaluatePromotion({ ...base, tier: 'working', type: 'fact', content: 'x' }, NOW, DEFAULT_POLICY);
        if (w !== 'episodic') throw new Error(`expected episodic, got ${w}`);
        const e = evaluatePromotion({ ...base, tier: 'episodic', type: 'fact', content: 'x' }, NOW, DEFAULT_POLICY);
        if (e !== 'semantic') throw new Error(`expected semantic, got ${e}`);
        const s = evaluatePromotion({ ...base, tier: 'semantic', type: 'skill', content: 'x' }, NOW, DEFAULT_POLICY);
        if (s !== 'procedural') throw new Error(`expected procedural, got ${s}`);
        const p = evaluatePromotion({ ...base, tier: 'procedural', type: 'skill', content: 'x' }, NOW, DEFAULT_POLICY);
        if (p !== null) throw new Error('procedural must be terminal');
      }),

      await this.check('G23.5', 'canDemote + nextTier sanity', () => {
        if (!canDemote('working') || !canDemote('episodic')) throw new Error('working/episodic must be demotable');
        if (canDemote('semantic') || canDemote('procedural')) throw new Error('semantic/procedural must not be demotable');
      ...
      await this.check('G23.6', 'MemoryTierStore enforces content_hash + scope idempotency (50 inserts -> 1 entry)', async () => {
        ...
        try {
          ...
          for (let i = 0; i < 50; i++) {
            store.insert({
              tier: 'working', type: 'fact', content: 'same content',
      ...
      await this.check('G23.7', 'promote into procedural makes entry immutable', async () => {
        ...
        try {
          ...
          const e = store.insert({
            tier: 'semantic', type: 'skill', content: 'how to ship Phase 5',
            sourceEntryIds: ['ce:1'], confidence: 0.9, scopeId: SCOPE, metadata: {},
          });
          store.promote(e.id, 'procedural');
          let caught = false;
          try { store.promote(e.id, 'semantic'); } catch (err) { caught = err instanceof ImmutableMemoryError; }
...
main().catch((e) => { console.error('G23 crashed:', e); process.exit(2); });
