...
function mkMem(i: number, tier: 'working' | 'episodic' | 'semantic' | 'procedural', text: string, confidence = 0.8): MemoryEntry {
  return {
    id: `mem:${String(i).padStart(2, '0').padEnd(32, 'A')}` as any,
    tier, type: 'fact', content: text,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G9.1', 'injection-strategies unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/injection-strategies'), stdio: 'pipe' });
      }),

      await this.check('G9.2', 'all 5 strategies registered (A/B/C/D/E)', () => {
        const names = Object.keys(STRATEGIES).sort();
        if (names.join(',') !== 'A,B,C,D,E') throw new Error(`expected A..E, got ${names.join(',')}`);
        for (const k of names) {
          if (STRATEGIES[k as keyof typeof STRATEGIES].name !== k) {
            throw new Error(`strategy ${k} reports name ${STRATEGIES[k as keyof typeof STRATEGIES].name}`);
          }
        }
      }),

      await this.check('G9.3', 'default strategy is B (System Prologue Tiered)', () => {
        if (DEFAULT_STRATEGY.name !== 'B') throw new Error(`default is ${DEFAULT_STRATEGY.name}, expected B`);
      }),
      ...
      await this.check('G9.4', 'B excludes semantic+procedural from system block', () => {
        ...
        if (sys.includes('tier S') || sys.includes('tier P')) throw new Error('B must exclude semantic/procedural');
      }),
      ...
      await this.check('G9.6', 'D injects assistant recall turn BEFORE the user message', () => {
        ...
        if (out.messages[idxUser - 1].role !== 'assistant') throw new Error('D did not insert assistant recall before user');
      }),
      ...
      await this.check('G9.7', 'E ranks by confidence descending', () => {
        const out = STRATEGIES.E.apply({
          ...
          memories: [
            mkMem(1, 'working', 'low',  0.30),
            mkMem(2, 'working', 'high', 0.95),
            mkMem(3, 'working', 'mid',  0.60),
          ],
          k: 2,
        });
        if (out.injectedMemoryIds.length !== 2) throw new Error(`expected 2 injected, got ${out.injectedMemoryIds.length}`);
        const sys = out.messages[0].content;
        if (!sys.includes('high') || !sys.includes('mid')) throw new Error('E did not pick top-2');
        if (sys.includes('low')) throw new Error('E should not include low-confidence below cutoff');
      }),
      ...
      await this.check('G9.8', 'all 5 strategies respect tokenBudget', () => {
        ...
        for (const s of Object.values(STRATEGIES)) {
          const out = s.apply({
            messages: [{ role: 'user', content: 'q' }],
            memories: big, tokenBudget: 200, k: 50,
          });
...
main().catch((e) => { console.error('G9 crashed:', e); process.exit(2); });
