    ...
    return [
      ...
      await this.check("G13.2", "each adapter uses CR v7.1 default model", () => {
        ...
        for (const c of cases) {
          if (c.adapter.model !== c.expected)
            throw new Error(`default model wrong: ${c.adapter.model} != ${c.expected}`);
        }
      }),
      ...
      await this.check("G13.3", "OpenAI rejects missing apiKey", () => {
        let caught = false;
        try {
          new OpenAiAdapter({ apiKey: "" } as any);
        } catch {
          caught = true;
        }
        ...
        let caught2 = false;
        try {
          new AnthropicAdapter({ apiKey: "" } as any);
        } catch {
          caught2 = true;
        }
        ...
        let caught3 = false;
        try {
          new GoogleAdapter({ apiKey: "" } as any);
        } catch {
          caught3 = true;
        }
      ...
      await this.check("G13.4", "Ollama uses default baseUrl localhost:11434", async () => {
        ...
        const a = new OllamaAdapter({
          fetchImpl: mockFetch(async (url) => {
            observedUrl = url;
            return new Response(
              JSON.stringify({ message: { content: "ok" }, done_reason: "stop" }),
              {
                status: 200,
        ...
        if (!observedUrl.startsWith("http://localhost:11434"))
          throw new Error(`unexpected baseUrl: ${observedUrl}`);
      }),

      await this.check(
        "G13.5",
        "custom baseUrl overrides default (OpenAI-compatible endpoint)",
        async () => {
          let observedUrl = "";
          ...
          await a.complete({ messages: [{ role: "user", content: "x" }] });
          if (!observedUrl.startsWith("https://api.together.xyz/v1"))
            throw new Error(`baseUrl override failed: ${observedUrl}`);
        },
      ),
    ];
  }
