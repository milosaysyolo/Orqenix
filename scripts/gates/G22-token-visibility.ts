...
class G22 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G22.yaml'), 'utf-8'); }
    ...
    return [
      await this.check('G22.1', 'METRIC_NAMES exports canonical strings', () => {
        ...
        for (const n of expected) {
          if (!Object.values(METRIC_NAMES).includes(n as any)) throw new Error(`missing canonical name: ${n}`);
        }
      }),
      ...
      await this.check('G22.2', 'summarizeMetrics aggregates per-strategy', () => {
        ...
        if (sum.perStrategyBreakdown.length !== 2) throw new Error('expected 2 strategies in breakdown');
      }),
...
async function main(): Promise<void> {
  const r = new G22(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G22 crashed:', e); process.exit(2); });
