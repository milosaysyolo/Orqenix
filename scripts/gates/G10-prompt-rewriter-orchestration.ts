...
async function freshStore(): Promise<{ store: MemoryTierStore; conn: SqliteConnection; dir: string }> {
...
function fakeAdapter(content: string, counter?: { n: number }): LlmAdapter {
  return {
    ...
    async complete(_req: LlmRequest): Promise<LlmResponse> {
      ...
      return { content, finishReason: 'stop', tokensIn: 1, tokensOut: 1, model: 'fake-1', provider: 'fake', latencyMs: 1 };
    },
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G10.1', 'prompt-rewriter unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/prompt-rewriter'), stdio: 'pipe' });
      }),
      ...
      await this.check('G10.2', 'default strategy is B', async () => {
        ...
        try {
          ...
          if (rw.getStrategyName() !== 'B') throw new Error(`default is ${rw.getStrategyName()}, expected B`);
        } finally { await cleanup(conn, dir); }
      }),

      await this.check('G10.3', 'rewrite injects matching memories', async () => {
        const { store, conn, dir } = await freshStore();
        try {
          store.insert({ tier: 'episodic', type: 'decision', content: 'I decided to use SQLite for storage',
          ...
          if (out.injectedMemoryIds.length === 0) throw new Error('expected at least 1 injected memory');
          const sys = out.messages.find((m) => m.role === 'system');
          if (!sys || !sys.content.includes('SQLite')) throw new Error('system block missing memory');
        } finally { await cleanup(conn, dir); }
      }),
      ...
      await this.check('G10.6', 'rewriteFn consolidates system message when enabled', async () => {
        ...
        try {
          ...
          const out = await rw.rewrite({
            messages: [{ role: 'system', content: 'base instructions' }, { role: 'user', content: 'q' }],
          });
          if (!out.rewriteApplied) throw new Error('rewrite was not applied');
          const sys = out.messages.find((m) => m.role === 'system');
          if (sys?.content !== 'CONSOLIDATED OUTPUT') throw new Error(`expected consolidated, got ${sys?.content}`);
        } finally { await cleanup(conn, dir); }
      }),

      await this.check('G10.7', 'adapter failure gracefully degrades (rewriteApplied=false)', async () => {
        const { store, conn, dir } = await freshStore();
        try {
...
main().catch((e) => { console.error('G10 crashed:', e); process.exit(2); });
