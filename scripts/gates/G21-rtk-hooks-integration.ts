...
class G21 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G21.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      ...
      await this.check('G21.2', 'blocks dangerous commands (rm, dd, shutdown, reboot, mkfs)', async () => {
        ...
        for (const cmd of blocked) {
          try { await runner.run(cmd, ['-rf', '/']); throw new Error(`${cmd} should have thrown`); }
          catch (e) { if (!(e instanceof RtkBlockedCommandError)) throw new Error(`${cmd}: expected RtkBlockedCommandError`); }
        }
      }),
      await this.check('G21.3', 'redacts API keys and Bearer tokens before output', () => {
        ...
        const withBearer = 'Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.dQw4w9WgXcQ';
        const redactedBearer = redact(withBearer, ['(?i)Bearer\\s+[A-Za-z0-9._-]+']);
        if (redactedBearer.includes('eyJhbGciOiJIUzI1NiJ9')) throw new Error('Bearer token not redacted');
      }),
      await this.check('G21.4', 'truncates stdout above configured cap', async () => {
        ...
        if (result.stdout.length > 2000) throw new Error(`stdout too long: ${result.stdout.length}`);
      }),
      await this.check('G21.5', 'records run counter and duration histogram metrics', async () => {
        ...
        if (result.durationMs <= 0) throw new Error(`expected positive duration, got ${result.durationMs}`);
        if (result.blocked) throw new Error('should not be blocked');
        if (result.timedOut) throw new Error('should not time out');
      }),
      await this.check('G21.6', 'extension formats output with exit code and truncation markers', () => {
        const result: RtkCommandResult = {
          cmd: 'echo', args: ['hello'], stdout: 'hello',
        ...
        const truncated: RtkCommandResult = {
          cmd: 'cat', args: ['big'], stdout: 'some data',
          stderr: '', exitCode: 0, durationMs: 5,
          truncatedStdout: true, truncatedStderr: false,
          blocked: false, timedOut: false,
        };
        const fmtTrunc = defaultFormatter(truncated);
        if (!fmtTrunc.includes('[stdout truncated]')) throw new Error('expected truncation marker');
        const runner = new RtkRunner();
...
async function main(): Promise<void> {
  const r = new G21(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G21 crashed:', e); process.exit(2); });
