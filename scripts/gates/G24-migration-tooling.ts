...
class G24 extends GateRunner {
  ...
  protected loadSpec(): unknown {
    return readFileSync(join(REPO_ROOT, ".orqenix/charter-gates/G24.yaml"), "utf-8");
  }
    ...
    return [
      ...
      await this.check("G24.2", "migrate applies all Phase 5 migrations in id order", async () => {
        ...
        try {
          ...
          for (let i = 1; i < ids.length; i++)
            if (ids[i] <= ids[i - 1]) throw new Error("not sorted");
      ...
      await this.check("G24.3", "migrate preserves Phase 4 legacy data", async () => {
        ...
        try {
          ...
          conn.close();
          if (rows.length !== 1 || rows[0].payload !== "legacy-row")
            throw new Error("legacy data lost");
        } finally {
          await teardown(dir);
        }
      }),
      await this.check("G24.4", "rollback restores from backup", async () => {
        ...
        try {
          ...
          if (status.currentPhase !== "phase-4")
            throw new Error(`expected phase-4 after rollback, got ${status.currentPhase}`);
        } finally {
          await teardown(dir);
        }
      }),
      ...
      await this.check("G24.7", "restoreFromBackup throws BackupMissingError", async () => {
        let caught = false;
        try {
          await restoreFromBackup("/nonexistent.bak", "/tmp/x.sqlite");
        } catch (e) {
          caught = e instanceof BackupMissingError;
        }
