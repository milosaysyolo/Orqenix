...
import { DropStrategy, DistillStrategy, SummarizeStrategy, CompressChainStrategy } from '@orqenix/compress-strategies';
...
class G16 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G16.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G16.1', 'plugin-compress-context unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/plugin-compress-context'), stdio: 'pipe' });
      }),
      ...
      await this.check('G16.2', 'v1 default export shape unchanged (run + ConfigSchema)', () => {
        ...
        if (typeof ConfigSchema.safeParse !== 'function') throw new Error('ConfigSchema is not zod');
      }),
      ...
      await this.check('G16.4', 'v1 above-threshold compression preserved (compressed=true, tokensOut < tokensIn)', async () => {
        const msgs = Array.from({ length: 30 }, (_, i) => ({
          role: i % 2 === 0 ? 'user' : 'assistant', content: 'x'.repeat(120),
        }));
        const r = await plugin.run({ context: msgs, threshold: 200 });
        if (!r.compressed) throw new Error('v1 should compress above threshold');
        if (r.metrics.tokensOut >= r.metrics.tokensIn) throw new Error('v1 tokensOut should be less than tokensIn');
      }),

      await this.check('G16.5', 'v2 path produces richer metrics with tier-0 preservation', async () => {
        ...
        if (r.metrics.preservedTier0Count !== 1) throw new Error(`expected 1 tier-0 preserved, got ${r.metrics.preservedTier0Count}`);
        if (!r.metrics.strategyId) throw new Error('v2 missing strategyId');
        if (r.context[0].content !== 'core locked') throw new Error('tier-0 system message was modified');
      }),

      await this.check('G16.6', 'v2 with already-small input is a no-op', async () => {
        const engine = new SmartCompressionEngine({
          config: { targetTokens: 1000, maxTokens: 3000 },
          strategies: { drop: new DropStrategy(), summarize: new SummarizeStrategy(), distill: new DistillStrategy({ extract: () => [] }), 'compress-chain': new CompressChainStrategy({ distill: new DistillStrategy({ extract: () => [] }), summarize: new SummarizeStrategy() }) },
          scopeId: SCOPE,
        });
        const v2 = createV2Plugin({ engine });
...
async function main(): Promise<void> {
  const r = new G16(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G16 crashed:', e); process.exit(2); });
