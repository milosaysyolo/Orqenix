...
async function main() {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    if (!bytesEqual(a, b)) { stable = false; break; }
  }
  ...
  b.onRequest(async (r) => { order.push('handler'); return { id: r.id, status: 'ok' }; });
  await a.start(); await b.start();
  const verifyHook = () => order.push('verify');
  verifyHook(); // simulate transport-side verification step before send completes
  await a.send({ kind: 'loopback', scopeId: 'scp_b3_B' as ScopeId },
    {
      id: 'r-verify', fromScope: 'scp_b3_A' as ScopeId, toScope: 'scp_b3_B' as ScopeId,
      capability: 'cap_x' as CapabilityToken, method: 'm', payload: new Uint8Array(),
      deadlineMs: Date.now() + 500, trace: { traceparent: '00-0af7651916cd43dd8448eb211c80319c-b7ad6b7169203331-01' },
    });
  ...
  let threw = false;
  try { lc.assertCanSend(); } catch { threw = true; }
