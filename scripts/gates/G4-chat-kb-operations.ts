...
import {
  ChatStore, CHAT_KB_MIGRATIONS, createChatVecTable,
  HashChainBrokenError, ChatWriteUnauthorizedError,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G4.1', 'append 50 entries and listEntries returns in order', async () => {
        ...
        } finally { conn.close(); await rm(dir, { recursive: true, force: true }); }
      }),
      ...
      await this.check('G4.2', 'verifyHashChain green on untampered chain', async () => {
        ...
        try {
          ...
          for (let i = 0; i < 20; i++) {
            await store.appendEntry({ sessionId: s.sessionId, role: 'user', content: `m${i}`, metadata: {} });
          }
      ...
      await this.check('G4.3', 'verifyHashChain catches tampered content', async () => {
        ...
        try {
          ...
          let caught = false;
          try { await store.verifyHashChain(s.sessionId); } catch (e) {
      ...
      await this.check('G4.4', 'capability gate: write:kb-chat token accepted', async () => {
        ...
        try {
          ...
          const { encoded } = await issueToken({
            issuerScopeId: iss, issuerPrivateKey: kp.privateKey,
            subjectScopeId: iss, audienceScopeId: iss,
            caps: ['write:kb-chat'], ttlSeconds: 60,
          });
          await store.appendEntry(
      ...
      await this.check('G4.5', 'capability gate: read-only token rejected', async () => {
        ...
        try {
          ...
          const { encoded } = await issueToken({
            issuerScopeId: iss, issuerPrivateKey: kp.privateKey,
            subjectScopeId: iss, audienceScopeId: iss,
            caps: ['read:kb-chat'], ttlSeconds: 60,
          });
          let caught = false;
          try {
            await store.appendEntry(
          ...
          } catch (e) { caught = e instanceof ChatWriteUnauthorizedError; }
      ...
      await this.check('G4.6', 'vector search finds nearest entry', async () => {
        ...
        try {
          ...
          const e2 = await store.appendEntry({ sessionId: s.sessionId, role: 'user', content: 'two', metadata: {} });
          const e3 = await store.appendEntry({ sessionId: s.sessionId, role: 'user', content: 'three', metadata: {} });
      ...
      await this.check('G4.7', 'session cascade delete removes entries', async () => {
        ...
        try {
          ...
          await store.appendEntry({ sessionId: s.sessionId, role: 'user', content: 'b', metadata: {} });
          conn.prepare(`DELETE FROM chat_sessions WHERE session_id = ?`).run(s.sessionId);
          const left = conn.prepare(`SELECT COUNT(*) as c FROM chat_entries WHERE session_id = ?`)
...
main().catch((e) => { console.error('G4 crashed:', e); process.exit(2); });
