...
async function fresh() {
  ...
  const ctx = {
    rootDir: dir, dbPath: join(dir, 'kb.sqlite'), scopeId: SCOPE,
    io: { stdout: () => {}, stderr: () => {} },
  };
  return { dir, ctx };
}
...
class G25 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G25.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      ...
      await this.check('G25.8', 'required flag enforcement (scope init without --name)', async () => {
        const { dir, ctx } = await fresh();
        try {
...
async function main(): Promise<void> {
  const r = new G25(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G25 crashed:', e); process.exit(2); });
