...
class G34 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G34.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G34.1', 'mesh-routing unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/mesh-routing'), stdio: 'pipe' });
      }),
      await this.check('G34.2', 'parallel fanout to all active outbound links', async () => {
        ...
        try {
          activate(linkStore, B); activate(linkStore, C);
      ...
      await this.check('G34.3', 'quorum = ceil(scopesQueried/2)', async () => {
        const { dir, conn, linkStore, transport, router } = await setup();
        try {
          activate(linkStore, B); activate(linkStore, C); activate(linkStore, D);
          transport.setHandler(B, async () => [hit(B, 'b', 1)]);
      ...
      await this.check('G34.4', 'timeout outcome recorded per-target', async () => {
        ...
        try {
          ...
          if (r.outcomes[0].ok === false && r.outcomes[0].reason !== 'timeout') throw new Error('reason not timeout');
        } finally { await tear(dir, conn); }
      }),
...
async function main(): Promise<void> {
  const r = new G34(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G34 crashed:', e); process.exit(2); });
