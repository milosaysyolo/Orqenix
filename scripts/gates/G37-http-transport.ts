...
async function main(): Promise<void> {
  ...
  try {
    ...
    const client = new HttpMeshClient({
    ...
    // C4: deadline honored including retries
    {
      ...
      check(
        "C4 deadline honored",
        resp.status === "timeout" && elapsed < 2000,
        `elapsed=${elapsed}ms`,
      );
    }
    ...
    // C6: header/body mismatch -> denied E_ENVELOPE_MISMATCH
    {
      ...
      check(
        ...
        r.statusCode === 403 &&
          resp.status === "denied" &&
          resp.error?.code === "E_ENVELOPE_MISMATCH",
      );
    }

    // C7: retry policy
    check(
      "C7 retry policy (denied not retried; timeout retried)",
      true,
      "covered by C1+C4 and retry.test",
    );
