...
import {
  issueToken, RevocationStore, TokenVerifier, TokenRevokedError,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G32.1', 'revocation tests pass', () => {
        execSync('pnpm --filter @orqenix/capability-tokens exec vitest run test/revocation.test.ts', { cwd: REPO_ROOT, stdio: 'pipe' });
      }),
      await this.check('G32.2', '30 revoke + isRevoked roundtrips', async () => {
        ...
        try {
          ...
          for (let i = 0; i < 30; i++) {
            ...
            if (!(await store.isRevoked(token.payload.jti as any))) throw new Error(`not detected iter ${i}`);
          }
      ...
      await this.check('G32.3', 'verifier rejects revoked token', async () => {
        ...
        try {
          ...
          const v = new TokenVerifier({ getIssuerPublicKey: async () => kp.publicKey, revocationStore: store });
          let caught = false;
          try { await v.verify(encoded, 'read:kb-docs'); }
          catch (e) { caught = e instanceof TokenRevokedError; }
...
main().catch((e) => { console.error('G32 crashed:', e); process.exit(2); });
