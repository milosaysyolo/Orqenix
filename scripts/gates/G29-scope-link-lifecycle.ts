...
import {
  ScopeLinkStore, SCOPE_LINK_MIGRATIONS,
  LinkAlreadyExistsError, LinkStateError,
...
class G29 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G29.yaml'), 'utf-8'); }
    ...
    return [
      ...
      await this.check('G29.2', 'duplicate (local, remote, direction) rejected', async () => {
        ...
        try {
          ...
          let caught = false;
          try { store.create({ remoteScopeId: B, direction: 'outbound' }); } catch (e) { caught = e instanceof LinkAlreadyExistsError; }
          if (!caught) throw new Error('duplicate not rejected');
        } finally { await tear(dir, conn); }
      }),
      await this.check('G29.3', 'state machine: pending -> active -> revoked, no other paths', async () => {
        const { dir, conn, store } = await fresh();
        try {
          store.create({ remoteScopeId: B, direction: 'outbound' });
          ...
          let caught = false;
          try { store.updateStatus(B, 'outbound', 'active'); } catch (e) { caught = e instanceof LinkStateError; }
      ...
      await this.check('G29.4', 'recordSync requires active state', async () => {
        ...
        try {
          store.create({ remoteScopeId: B, direction: 'outbound' });
          let caught = false;
          try { store.recordSync(B, 'outbound'); } catch (e) { caught = e instanceof LinkStateError; }
      ...
      await this.check('G29.5', 'self-link rejected at creation', async () => {
        ...
        try {
          let caught = false;
          try { store.create({ remoteScopeId: A, direction: 'outbound' }); } catch { caught = true; }
...
async function main(): Promise<void> {
  const r = new G29(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G29 crashed:', e); process.exit(2); });
