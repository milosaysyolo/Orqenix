...
class G19 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G19.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G19.1', 'file-watcher unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/file-watcher'), stdio: 'pipe' });
      }),
      await this.check('G19.2', 'detects add+change+unlink end-to-end', async () => {
        ...
        await watcher.start(async (b) => { batches.push(b); });
        try {
      ...
      await this.check('G19.3', 'debounce collapses bursts', async () => {
        ...
        try {
          ...
          if (batches.length > 2) throw new Error(`expected debounced batches (<=2), got ${batches.length}`);
        } finally {
          await watcher.stop();
          await new Promise((r) => setTimeout(r, 50));
          await rm(dir, { recursive: true, force: true, maxRetries: 3 });
      ...
      await this.check('G19.4', 'ignores .git and node_modules', async () => {
        ...
        try {
          ...
          if (paths.some((p) => p.startsWith('node_modules/'))) throw new Error('node_modules not ignored');
...
async function main(): Promise<void> {
  const r = new G19(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G19 crashed:', e); process.exit(2); });
