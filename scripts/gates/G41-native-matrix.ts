...
async function main(): Promise<void> {
  ...
  const expectedTier1: Array<[string, number]> = [
    ['darwin-arm64', 22], ['darwin-arm64', 24],
    ['linux-x64-gnu', 22], ['linux-x64-gnu', 24],
    ['win32-x64', 22], ['win32-x64', 24],
  ];
  const expectedTier2: Array<[string, number]> = [
    ['darwin-x64', 22], ['darwin-x64', 24],
    ['linux-arm64-gnu', 22], ['linux-arm64-gnu', 24],
  ];
  ...
  const nodes = new Set(include.map((e) => e.node).filter((n): n is number => typeof n === 'number'));
  check(
  ...
  const usesList = (job?.steps ?? []).map((s) => s.uses).filter((u): u is string => typeof u === 'string');
  const required = [
    'actions/checkout@v4',
  ...
  if (process.env.SKIP_LOCAL_SMOKE === '1') {
    ...
    if (fixtureOk) {
      const r = spawnSync(process.execPath, ['--experimental-vm-modules', SMOKE_PATH], { stdio: 'inherit' });
