...
class G18 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G18.yaml'), 'utf-8'); }
    ...
    return [
      ...
      await this.check('G18.3', '30 random payload tampers all detected', async () => {
        for (let iter = 0; iter < 30; iter++) {
          const { dir, conn, store } = await fresh();
          try {
            for (let i = 0; i < 10; i++) store.append({ actorScopeId: A, eventKind: 'mesh_query_run', payload: { i } });
            const target = 1 + Math.floor(Math.random() * 10);
            conn.prepare(`UPDATE audit_log_entries SET payload_json = '{"tampered":${iter}}' WHERE rowid = ?`).run(target);
            let caught = false;
            try { store.verifyChain(); } catch (e) { caught = e instanceof AuditChainBrokenError; }
      ...
      await this.check('G18.4', 'broken prevHash linkage detected', async () => {
        const { dir, conn, store } = await fresh();
        try {
          for (let i = 0; i < 5; i++) store.append({ actorScopeId: A, eventKind: 'mesh_query_run', payload: { i } });
          conn.prepare(`UPDATE audit_log_entries SET prev_hash = '${'f'.repeat(64)}' WHERE rowid = 3`).run();
          let caught = false;
          try { store.verifyChain(); } catch (e) { caught = e instanceof AuditChainBrokenError; }
...
async function main(): Promise<void> {
  const r = new G18(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G18 crashed:', e); process.exit(2); });
