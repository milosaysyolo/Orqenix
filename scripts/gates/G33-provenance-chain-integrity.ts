...
import {
  rootTag, appendTag, verifyChain, computeChainHash,
...
function tag(over: Partial<ProvenanceTag> = {}): Omit<ProvenanceTag, 'parentChainHash'> {
  return { sourceScopeId: A, producedAt: '2026-06-02T00:00:00Z', sourceKind: 'local', ...over } as Omit<ProvenanceTag, 'parentChainHash'>;
}

class G33 extends GateRunner {
  readonly id = 'G33';
  readonly title = 'Provenance Chain Integrity';
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G33.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      ...
      await this.check('G33.2', '100x random chains verify cleanly', () => {
        for (let i = 0; i < 100; i++) {
          let chain = rootTag(tag({ sourceScopeId: A, producedAt: new Date(2026, 5, 1, 0, 0, i).toISOString() }));
          const len = 1 + (i % 8);
          for (let j = 0; j < len; j++) {
            chain = appendTag(chain, tag({
      ...
      await this.check('G33.3', '50x tampered chains all detected', () => {
        for (let i = 0; i < 50; i++) {
          ...
          const tamperedTags = chain.tags.map((t, idx) => idx === 0 ? { ...t, originPath: `tamper-${i}` } : t) as ProvenanceTag[];
          const tampered = { tags: tamperedTags, chainHash: chain.chainHash };
          let caught = false;
          try { verifyChain(tampered); } catch (e) { caught = e instanceof ProvenanceChainBrokenError; }
...
async function main(): Promise<void> {
  const r = new G33(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G33 crashed:', e); process.exit(2); });
