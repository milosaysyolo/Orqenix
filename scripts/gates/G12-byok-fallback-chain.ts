...
import {
  ...
  LlmProviderError, LlmRateLimitError, LlmAuthError, LlmTimeoutError,
...
function adapter(name: string, fn: () => Promise<LlmResponse>): LlmAdapter {
  return {
    provider: name, model: 'mock',
    async complete(_req: LlmRequest): Promise<LlmResponse> { return fn(); },
    ...
    return [
      await this.check('G12.1', 'uses first healthy adapter', async () => {
        const chain = new FallbackChain({
          adapters: [
            adapter('a', async () => ({ content: 'A', finishReason: 'stop', tokensIn: 0, tokensOut: 0, model: 'm', provider: 'a', latencyMs: 1 })),
            adapter('b', async () => ({ content: 'B', finishReason: 'stop', tokensIn: 0, tokensOut: 0, model: 'm', provider: 'b', latencyMs: 1 })),
          ],
        });
      ...
      await this.check('G12.2', 'falls through LlmProviderError', async () => {
        const chain = new FallbackChain({
          adapters: [
            adapter('a', async () => { throw new LlmProviderError('a', 'down'); }),
            adapter('b', async () => ({ content: 'B', finishReason: 'stop', tokensIn: 0, tokensOut: 0, model: 'm', provider: 'b', latencyMs: 1 })),
          ],
        });
        const r = await chain.complete({ messages: [{ role: 'user', content: 'x' }] });
      ...
      await this.check('G12.3', 'falls through LlmRateLimitError', async () => {
        const chain = new FallbackChain({
          adapters: [
            adapter('a', async () => { throw new LlmRateLimitError('a'); }),
            adapter('b', async () => ({ content: 'B', finishReason: 'stop', tokensIn: 0, tokensOut: 0, model: 'm', provider: 'b', latencyMs: 1 })),
          ],
        });
        const r = await chain.complete({ messages: [{ role: 'user', content: 'x' }] });
      ...
      await this.check('G12.4', 'falls through LlmAuthError and LlmTimeoutError', async () => {
        const chain = new FallbackChain({
          adapters: [
            adapter('a', async () => { throw new LlmAuthError('a'); }),
            adapter('b', async () => { throw new LlmTimeoutError(100); }),
            adapter('c', async () => ({ content: 'C', finishReason: 'stop', tokensIn: 0, tokensOut: 0, model: 'm', provider: 'c', latencyMs: 1 })),
          ],
        });
        const r = await chain.complete({ messages: [{ role: 'user', content: 'x' }] });
      ...
      await this.check('G12.5', 'aggregates errors when all fail', async () => {
        const chain = new FallbackChain({
          adapters: [
            adapter('a', async () => { throw new LlmProviderError('a', 'down'); }),
            adapter('b', async () => { throw new LlmRateLimitError('b'); }),
          ],
        });
        let caught = false;
        try { await chain.complete({ messages: [{ role: 'user', content: 'x' }] }); }
        catch (e) {
          caught = e instanceof LlmProviderError && /all 2 adapters failed/.test((e as Error).message);
        }
      ...
      await this.check('G12.6', 'empty chain rejected at construction', () => {
        let caught = false;
        try { new FallbackChain({ adapters: [] }); }
        catch { caught = true; }
...
main().catch((e) => { console.error('G12 crashed:', e); process.exit(2); });
