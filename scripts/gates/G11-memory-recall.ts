...
async function setup(): Promise<{ store: MemoryTierStore; recall: KeywordRecall; conn: SqliteConnection; dir: string }> {
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G11.1', 'recall returns matches for keyword query', async () => {
        ...
        try {
          store.insert({ tier: 'episodic', type: 'preference', content: 'I prefer Rust for runtime',
            sourceEntryIds: ['ce:1'], confidence: 0.9, scopeId: SCOPE, metadata: {} });
        ...
        } finally { await teardown(conn, dir); }
      }),

      await this.check('G11.2', 'recall respects tier filter', async () => {
        const { store, recall, conn, dir } = await setup();
        try {
          store.insert({ tier: 'working', type: 'observation', content: 'noticed rust crate updates',
            sourceEntryIds: ['ce:1'], confidence: 0.4, scopeId: SCOPE, metadata: {} });
          store.insert({ tier: 'semantic', type: 'learning', content: 'rust has zero-cost abstractions',
            sourceEntryIds: ['ce:2'], confidence: 0.95, scopeId: SCOPE, metadata: {} });
          const onlyWorking = recall.recall('rust', { k: 5, tiers: ['working'] });
          const onlySemantic = recall.recall('rust', { k: 5, tiers: ['semantic'] });
          if (onlyWorking.length !== 1 || onlyWorking[0].tier !== 'working') throw new Error('tier filter failed for working');
          if (onlySemantic.length !== 1 || onlySemantic[0].tier !== 'semantic') throw new Error('tier filter failed for semantic');
        } finally { await teardown(conn, dir); }
      }),

      await this.check('G11.3', 'higher confidence ranks higher for same keyword count', async () => {
        const { store, recall, conn, dir } = await setup();
        try {
          store.insert({ tier: 'working', type: 'fact', content: 'rust is good',
            sourceEntryIds: ['ce:1'], confidence: 0.40, scopeId: SCOPE, metadata: {} });
          store.insert({ tier: 'working', type: 'fact', content: 'rust is great',
            sourceEntryIds: ['ce:2'], confidence: 0.95, scopeId: SCOPE, metadata: {} });
      ...
      await this.check('G11.4', 'recall increments accessCount on returned memories', async () => {
        const { store, recall, conn, dir } = await setup();
        try {
          const e = store.insert({ tier: 'working', type: 'fact', content: 'rust is fast',
            sourceEntryIds: ['ce:1'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
          const before = store.getById(e.id).accessCount;
          recall.recall('rust', { k: 5 });
          const after = store.getById(e.id).accessCount;
          if (after <= before) throw new Error(`accessCount not incremented: ${before} -> ${after}`);
        } finally { await teardown(conn, dir); }
      }),
      ...
      await this.check('G11.5', 'empty query returns empty result', async () => {
        ...
        try {
          if (recall.recall('', { k: 5 }).length !== 0) throw new Error('empty query should return empty');
          if (recall.recall('a', { k: 5 }).length !== 0) throw new Error('too-short tokens should be ignored');
        } finally { await teardown(conn, dir); }
      }),
      ...
      await this.check('G11.6', 'recall returns at most k', async () => {
        ...
        try {
          for (let i = 0; i < 20; i++) {
            store.insert({ tier: 'working', type: 'fact', content: `rust thing ${i}`,
              sourceEntryIds: [`ce:${i}`], confidence: 0.8, scopeId: SCOPE, metadata: {} });
          }
...
main().catch((e) => { console.error('G11 crashed:', e); process.exit(2); });
