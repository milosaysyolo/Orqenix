...
import {
  issueToken, delegateToken, TokenVerifier,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G28.1', 'signing tests pass', () => {
        execSync('pnpm --filter @orqenix/capability-tokens exec vitest run test/signing.test.ts', { cwd: REPO_ROOT, stdio: 'pipe' });
      }),
      await this.check('G28.2', '50 fresh sign/verify roundtrips', async () => {
        for (let i = 0; i < 50; i++) {
          ...
          const { encoded } = await issueToken({
            ...
            caps: ['read:kb-docs'], ttlSeconds: 60,
          });
      ...
      await this.check('G28.3', '50 tampers all rejected', async () => {
        for (let i = 0; i < 50; i++) {
          ...
          const v = new TokenVerifier({ getIssuerPublicKey: async () => iss.publicKey });
          let caught = false;
          try { await v.verify(tampered, 'read:kb-docs'); } catch { caught = true; }
      ...
      await this.check('G28.4', 'delegation chain depth respected', async () => {
        ...
        const { token: p } = await issueToken({
          ...
          caps: ['delegate:*', 'read:*'], ttlSeconds: 3600, maxDelegationDepth: 1,
        });
        const { token: child } = await delegateToken({
          parentToken: p, parentPrivateKey: mid.privateKey,
          newSubjectScopeId: leaf.scopeId, caps: ['read:kb-docs'], ttlSeconds: 60,
        });
      ...
      await this.check('G28.5', 'expired token rejected with skew applied', async () => {
        ...
        t += 3600;
        const v = new TokenVerifier({ getIssuerPublicKey: async () => iss.publicKey, now: () => t, clockSkewSeconds: 30 });
        let caught = false;
        try { await v.verify(encoded, 'read:kb-docs'); } catch { caught = true; }
...
main().catch((e) => { console.error('G28 crashed:', e); process.exit(2); });
