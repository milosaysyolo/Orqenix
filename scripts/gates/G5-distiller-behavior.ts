...
class G5 extends GateRunner {
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
        ...
        try {
          ...
          for (let i = 0; i < 20; i++) {
            await chat.appendEntry({
              ...
              role: "user",
              content: `I decided to use approach-${i}`,
              metadata: {},
            });
          }
      ...
      await this.check(
        "G5.4",
        "idempotent across content_hash (no duplicate memories)",
        async () => {
          ...
          try {
            ...
            if (prefs.length !== 1)
              throw new Error(`expected 1 dedup-ed preference, got ${prefs.length}`);
            conn.close();
          } finally {
            await rm(dir, { recursive: true, force: true });
          }
      ...
      await this.check("G5.5", "minConfidence filter respected", async () => {
        ...
        try {
          ...
          await chat.appendEntry({
            sessionId: s.sessionId,
            role: "user",
            content: "maybe I should consider X",
            metadata: {},
          });
