...
import { ReindexIndexer, Reindexer, type ReindexStats, type IndexEntry } from '@orqenix/reindex-incremental';
...
class G20 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G20.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G20.1', 'reindex-incremental unit tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/reindex-incremental'), stdio: 'pipe' });
      }),
      await this.check('G20.2', 'scanFull adds, updates, removes, marks unchanged correctly', async () => {
        ...
        try {
          ...
          if (s1.filesUpdated !== 0) throw new Error(`expected 0 updated, got ${s1.filesUpdated}`);
          if (s1.filesRemoved !== 0) throw new Error(`expected 0 removed, got ${s1.filesRemoved}`);
          if (s1.filesUnchanged !== 0) throw new Error(`expected 0 unchanged, got ${s1.filesUnchanged}`);
          const s2 = await reindexer.scanFull();
          if (s2.filesUnchanged !== 3) throw new Error(`expected 3 unchanged, got ${s2.filesUnchanged}`);
          await writeFile(join(dir, 'a.txt'), 'aaa-modified');
          const s3 = await reindexer.scanFull();
          if (s3.filesUpdated !== 1) throw new Error(`expected 1 updated, got ${s3.filesUpdated}`);
          if (s3.filesUnchanged !== 2) throw new Error(`expected 2 unchanged, got ${s3.filesUnchanged}`);
          await rm(join(dir, 'b.txt'));
          const s4 = await reindexer.scanFull();
          if (s4.filesRemoved !== 1) throw new Error(`expected 1 removed, got ${s4.filesRemoved}`);
      ...
      await this.check('G20.3', 'applyEvents is idempotent on re-apply', async () => {
        ...
        try {
          ...
          const stats = await reindexer.applyEvents([
            { kind: 'change', path: join(dir, 'x.txt'), relPath: 'x.txt', timestamp: new Date().toISOString() },
          ]);
          if (stats.filesUnchanged !== 1) throw new Error(`expected 1 unchanged on re-apply, got ${stats.filesUnchanged}`);
          if (stats.filesUpdated !== 0) throw new Error(`expected 0 updated on re-apply, got ${stats.filesUpdated}`);
        } finally {
          await rm(dir, { recursive: true, force: true, maxRetries: 3 });
          conn.close();
        }
      ...
      await this.check('G20.4', 'ignores .git and node_modules paths', async () => {
        ...
        try {
          ...
          if (stats.filesScanned !== 1) throw new Error(`expected 1 scanned, got ${stats.filesScanned}`);
      ...
      await this.check('G20.5', 'content_hash drives update detection (mtime alone insufficient)', async () => {
        const conn = createConn();
        const indexer = new ReindexIndexer(conn);
        const SCOPE = 'scope:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
        try {
          indexer.upsert({
            relPath: 'f.txt', scopeId: SCOPE,
            contentHash: hashString('original content'),
            sizeBytes: 16, modifiedAt: '2024-01-01T00:00:00.000Z',
          });
          const prior1 = indexer.get(SCOPE, 'f.txt');
          indexer.upsert({
            relPath: 'f.txt', scopeId: SCOPE,
            contentHash: prior1!.contentHash,
            sizeBytes: 16, modifiedAt: '2025-01-01T00:00:00.000Z',
          });
          if (indexer.count(SCOPE) !== 1) throw new Error('same hash should not create duplicate');
          indexer.upsert({
            relPath: 'f.txt', scopeId: SCOPE,
            contentHash: hashString('changed content'),
            sizeBytes: 16, modifiedAt: '2025-01-01T00:00:00.000Z',
          });
          if (indexer.count(SCOPE) !== 1) throw new Error('different hash should update, not insert');
          const after = indexer.get(SCOPE, 'f.txt');
          if (after!.contentHash !== hashString('changed content')) throw new Error('content hash was not updated');
        } finally {
          conn.close();
        }
      }),
...
async function main(): Promise<void> {
  const r = new G20(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G20 crashed:', e); process.exit(2); });
