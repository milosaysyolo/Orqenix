...
async function main(): Promise<void> {
  ...
  {
    const t1 = new Libp2pMeshTransport({ localScopeId: 'scp_b3_aa' as ScopeId, scopeSeed: mkSeed(5) });
    const t2 = new Libp2pMeshTransport({ localScopeId: 'scp_b3_aa' as ScopeId, scopeSeed: mkSeed(5) });
    await t1.start();
    await t2.start();
  ...
  {
    const A = new Libp2pMeshTransport({ localScopeId: 'scp_b3_aa' as ScopeId, scopeSeed: mkSeed(1) });
    const B = new Libp2pMeshTransport({ localScopeId: 'scp_b3_bb' as ScopeId, scopeSeed: mkSeed(2) });
    B.onRequest(async (r) => ({ id: r.id, status: 'ok', payload: new Uint8Array([...r.payload, 0xee]) }));
    await A.start();
    await B.start();
  ...
  // ---- C5: rejected capability handshake yields denied, no handler invocation ----
  {
    ...
    let handlerCalls = 0;
    B.onRequest(async (r) => { handlerCalls++; return { id: r.id, status: 'ok' }; });
    await A.start();
    await B.start();
    const addr: MeshAddress = { kind: 'libp2p', multiaddr: B.multiaddrs()[0] };
  ...
  {
    const t = new Libp2pMeshTransport({ localScopeId: 'scp_b3_aa' as ScopeId, scopeSeed: mkSeed(3) });
    await t.start();
    await t.start();
    await t.stop();
    await t.stop();
  ...
  {
    const t = new Libp2pMeshTransport({ localScopeId: 'scp_b3_aa' as ScopeId, scopeSeed: mkSeed(4) });
    let threw = false;
    try {
      await t.send({ kind: 'libp2p', multiaddr: '/memory/none' }, mkReq('scp_b3_zz' as ScopeId, '01HV0R6X3M8YQ9G7F2D5W1ZA07'));
    } catch {
      threw = true;
    }
  ...
  // ---- C8: deadline honored; expired request returns timeout ----
  {
    ...
    await B.start();
    const addr: MeshAddress = { kind: 'libp2p', multiaddr: B.multiaddrs()[0] };
    const resp = await A.send(addr, mkReq('scp_b3_bb' as ScopeId, '01HV0R6X3M8YQ9G7F2D5W1ZA08', Date.now() + 80));
