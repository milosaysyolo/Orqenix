...
import {
  encodeFull, decodeFull, encodeDelta, applyDelta,
  hashBytes, reconstructFromChain, type DiffEntry, type ContentHash,
  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      ...
      await this.check('G2.5', '20-entry chain reconstructs to final state', async () => {
        const versions = Array.from({ length: 20 }, (_, i) => `version ${i}: ` + 'x'.repeat(i * 10));
        ...
        for (let i = 0; i < versions.length; i++) {
          ...
          if (i === 0) {
            ...
            entries.push({
              entryId: `e${i}`, baseHash: null, contentHash: ch, encoding: 'full',
              payload, sizeBytes: payload.length, createdAt: `2026-01-01T00:00:${String(i).padStart(2,'0')}Z`,
            });
          } else {
            const payload = await encodeDelta(prevBytes!, bytes);
            entries.push({
              entryId: `e${i}`, baseHash: prevHash, contentHash: ch, encoding: 'zstd-delta',
              payload, sizeBytes: payload.length, createdAt: `2026-01-01T00:00:${String(i).padStart(2,'0')}Z`,
            });
          }
          prevBytes = bytes;
      ...
      await this.check('G2.6', 'BLAKE3 content hash determinism (200 runs)', () => {
        ...
        for (let i = 0; i < 200; i++) if (hashBytes(bytes) !== first) throw new Error(`non-det iter ${i}`);
      }),
    ];
  }
...
main().catch((e) => { console.error('G2 crashed:', e); process.exit(2); });
