...
class G30 extends GateRunner {
  ...
  protected loadSpec(): unknown {
    return readFileSync(join(REPO_ROOT, ".orqenix/charter-gates/G30.yaml"), "utf-8");
  }
    ...
    return [
      ...
      await this.check("G30.3", "destructive op requires valid confirmation token", async () => {
        ...
        try {
          ...
          if (idKey !== "PEM-PLACEHOLDER")
            throw new Error("identity.key was mutated despite bad token");
        } finally {
          await tear(dir, conn);
        }
      }),
