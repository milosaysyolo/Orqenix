...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `    Rewriter injected ${out.injectedMemoryIds.length} memory ids, strategy=${out.strategy}`,
    );
    ...
    console.log(
      `    Unlink executed at ${report.executedAt}, verifierChainHash=${report.verifierChainHash?.slice(0, 12)}...`,
    );
