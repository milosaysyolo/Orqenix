  ...
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G7.1', 'throttle unit tests pass', () => {
        execSync('npx vitest run test/throttle.test.ts', { cwd: join(REPO_ROOT, 'packages/memory-distiller'), stdio: 'pipe' });
      }),
      ...
      await this.check('G7.5', 'reset clears the baseline', async () => {
        ...
        t.reset();
        const m = t.measure();
        if (m.elapsedMs > 60) throw new Error(`reset did not clear baseline: elapsed=${m.elapsedMs}`);
      }),
    ];
  }
...
main().catch((e) => { console.error('G7 crashed:', e); process.exit(2); });
