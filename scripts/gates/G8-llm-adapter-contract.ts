...
class G8 extends GateRunner {
  ...
  protected loadSpec(): unknown { return readFileSync(join(REPO_ROOT, '.orqenix/charter-gates/G8.yaml'), 'utf-8'); }
  protected async runChecks(): Promise<GateCheck[]> {
    return [
      await this.check('G8.1', 'ollama adapter tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/llm-adapter-ollama'), stdio: 'pipe' });
      }),
      await this.check('G8.2', 'byok adapter tests pass', () => {
        execSync('npx vitest run', { cwd: join(REPO_ROOT, 'packages/llm-adapter-byok'), stdio: 'pipe' });
      }),
      await this.check('G8.3', 'all 5 adapter classes export LlmAdapter shape', () => {
        for (const cls of [OllamaAdapter, OpenAiAdapter, AnthropicAdapter, GoogleAdapter, DeepSeekAdapter]) {
          ...
          for (const m of ['complete', 'isHealthy']) {
            if (typeof proto[m] !== 'function') throw new Error(`adapter ${cls.name} missing ${m}()`);
          }
        }
      }),
    ];
...
async function main(): Promise<void> {
  const r = new G8(); const rep = await r.execute(); r.printSummary(rep);
  process.exit(rep.status === 'pass' ? 0 : 1);
}
main().catch((e) => { console.error('G8 crashed:', e); process.exit(2); });
