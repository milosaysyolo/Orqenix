...
test("14 reference plugins exist (G70)", async () => {
  const plugins = [
    ...
    "claude-code-binding-ref",
  ];
  for (const p of plugins) {
    const pkgPath = join(ROOT, "plugins", p, "package.json");
    if (!existsSync(pkgPath))
      throw new Error(`Reference plugin missing: plugins/${p}/package.json`);
  }
});
...
test("No tracked dist/ files", () => {
  let tracked = "";
  try {
    tracked = execSync('git ls-files "**/dist/**"', { cwd: ROOT, encoding: "utf-8" });
  } catch {
    /* no matches */
  }
...
test("No tracked node_modules", () => {
  let tracked = "";
  try {
    tracked = execSync('git ls-files "**/node_modules/**"', { cwd: ROOT, encoding: "utf-8" });
  } catch {
    /* no matches */
  }
  if (tracked.trim().length > 0) {
...
test("No .eslintrc.cjs files (must be flat config)", () => {
  let legacy = "";
  try {
    legacy = execSync('git ls-files "**/.eslintrc.cjs" "**/.eslintrc.js"', {
      cwd: ROOT,
      encoding: "utf-8",
    });
  } catch {
...
test("SPDX headers on all source TS files", async () => {
  const files = execSync(
    'git ls-files "packages/*/src/**/*.ts" "apps/*/src/**/*.ts" "plugins/*/src/**/*.ts"',
...
test("No PHASE 8 TODO markers in shipped code", () => {
  let matches = "";
  try {
    matches = execSync('git grep -l "PHASE 8 TODO" -- "packages/" "apps/" "plugins/"', {
      cwd: ROOT,
      encoding: "utf-8",
    });
  } catch {
