...
const workspaceNames = JSON.parse(workspacePkgs)
  .map((p) => p.name)
  .filter(Boolean);
...
for (const name of workspaceNames) {
  ...
  const relImporter =
    importerPath === ROOT ? "." : importerPath.replace(ROOT + "/", "").replace(ROOT + "\\", "");
  fsToLock.set(name, relImporter);
}

// Importers in lockfile but not on filesystem = phantom
const phantomImporters = importerKeys.filter(
  (k) => k !== "." && !Array.from(fsToLock.values()).includes(k),
);
