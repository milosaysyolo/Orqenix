...
check("Cleanup test suite 12+/13 pass", () => {
  const r = spawnSync("node", ["scripts/cleanup/cleanup.test.mjs"], {
    cwd: ROOT,
    encoding: "utf-8",
  });
...
check("Lockfile is in sync", () => {
  const r = spawnSync(
    "pnpm",
    ["install", "--frozen-lockfile", "--lockfile-only", "--reporter=silent"],
    {
      cwd: ROOT,
      shell: process.platform === "win32",
      stdio: "pipe",
...
check("No PHASE 8 TODO markers in shipped code", () => {
  try {
    const r = spawnSync(
      "git",
      ["grep", "-l", "PHASE 8 TODO", "--", "packages/", "apps/", "plugins/"],
      {
        cwd: ROOT,
        encoding: "utf-8",
      },
    ...
    /* git grep returns non-zero when nothing found, which is what we want */
  }
});
