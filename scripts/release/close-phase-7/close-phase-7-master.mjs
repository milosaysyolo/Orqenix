...
function blockSecurity(state) {
  ...
  if (!existsSync(cloudRepo)) {
    log(
      C.y(" Cloud repo not local \u2014 security tests target @orqenix-cloud/* (run on Cloud CI)"),
    );
  ...
  const checks = [
    [
      "audit-chain-tamper",
      ["pnpm", "--filter", "@orqenix-cloud/audit-core", "test", "--", "--grep", "tamper"],
    ],
    [
      "migration-300-strict",
      ["pnpm", "--filter", "@orqenix-cloud/identity-bridge", "test", "migration-300.test.ts"],
    ],
    [
      "oauth-scope-lock",
      ["pnpm", "--filter", "@orqenix-cloud/identity-bridge", "test", "github-provider.test.ts"],
    ],
    [
      "tls-required",
      [
    ...
    [
      "cross-tenant",
      ["pnpm", "--filter", "@orqenix-cloud/relay-core", "test", "router.integration.test.ts"],
    ],
  ];
  let fails = [];
...
function block3(state) {
  ...
  if (cer.status !== 0 && !PLAN) {
    checkpoint(state, "3", "RED", "ceremony.sh failed");
    return false;
  }
...
function block5(state) {
  ...
  for (const title of deferred) {
    ...
    const r = run("gh", [
      ...
      `[deferred] ${title}`,
      "--body",
      `Deferred from Phase 7 closure (release v0.7.0). Milestone ${milestone}.`,
      "--label",
      "deferred,phase-7.1",
      "--milestone",
      milestone,
...
function block7(state) {
  ...
  writeFileSync(
    resolve(OUT, "announce-checklist.md"),
    [
  ...
  if (closureReady) {
    ...
    writeFileSync(
      resolve(OUT, "closure-memory.txt"),
...
log(
  `Closure: release=${VERSION} tag=${TAG} range=${RANGE} from=${FROM} to=${TO} plan=${PLAN} host=${platform()}`,
);
...
const closed = ["0", "1", "2", "6sec", "3", "4", "5", "7"].every(
