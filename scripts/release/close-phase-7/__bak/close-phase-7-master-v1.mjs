...
function block1(state) {
  ...
  if (existsSync(rtt) && readFileSync(rtt, 'utf8').split('\n').length < 25)
    issues.push('rtt.bench.ts looks like a scaffold (<25 lines) \u2014 implement instrumented bench');
  ...
  if (issues.length > 0) {
    ...
    log(C.y('REMEDIATION: implement instrumented bench files measuring real p95 (rtt) and sustained env/s (throughput). Re-run block 1.'));
    return false;
  }
...
function block2(state) {
  ...
  if (existsSync(proRepo)) {
    ...
    if ([inst, build, test].some(x => x.status !== 0)) {
      ...
      checkpoint(state, '2', 'PARTIAL', 'fix-pro-deps OK; build fails pre-publication (expected). Run pnpm install after publish.');
      return true;
    }
    checkpoint(state, '2', 'GREEN', 'Pro deps fixed + build/test OK');
  } else {
    checkpoint(state, '2', 'PARTIAL', 'Pro repo not local \u2014 fix will run in CI before publish');
  }
  return true;
}
...
function blockSecurity(state) {
  ...
  const checks = [
    { name: 'audit-chain-tamper', cmd: ['pnpm', '--filter', '@orqenix-cloud/audit-core', 'test', '--', '--grep', 'tamper'] },
    { name: 'migration-300-strict', cmd: ['pnpm', '--filter', '@orqenix-cloud/identity-bridge', 'test', 'migration-300.test.ts'] },
    { name: 'oauth-scope-lock', cmd: ['pnpm', '--filter', '@orqenix-cloud/identity-bridge', 'test', 'github-provider.test.ts'] },
    { name: 'tls-required', cmd: ['pnpm', '--filter', '@orqenix-cloud/relay-transport', 'test', 'transport.integration.test.ts'] },
    { name: 'cross-tenant', cmd: ['pnpm', '--filter', '@orqenix-cloud/relay-core', 'test', 'router.integration.test.ts'] },
  ];
  let fails = [];
  ...
  const grep = run('grep', ['-rE', 'libp2p|dht|kad-dht', 'packages/relay-core/src', 'packages/relay-transport/src']);
...
function block5(state) {
  ...
  for (const title of deferred) {
    ...
    const r = run('gh', ['issue', 'create', '--title', `[deferred] ${title}`, '--body', `Deferred from Phase 7 closure. Milestone ${milestone}.`, '--label', 'deferred,phase-7.1', '--milestone', milestone]);
  ...
  checkpoint(state, '5', created.length === deferred.length ? 'GREEN' : 'PARTIAL', `${created.length}/${deferred.length} issues filed`);
  return true;
}
...
function block7(state) {
  ...
  const md = ['# Phase 7 Closure \u2014 Final Tasks', '',
    ...tasks.map((t, i) => `- [ ] ${t}`), ''];
...
// Driver
// ============================================================================
const BLOCKS = { 1: block1, 2: block2, 6: blockSecurity, 3: block3, 4: block4, 5: block5, 7: block7 };
// Execution sequence (note 6sec runs between 2 and 3)
const SEQ = [1, 2, 6, 3, 4, 5, 7];

const state = loadState();
log(`Closure run: version=${VERSION} from=${FROM} to=${TO} plan=${PLAN} strict=${STRICT} host=${platform()}`);
...
for (const b of SEQ) {
  ...
  if (!ok) { allGreen = false; if (STRICT) { log(C.r(`Stopping: block ${b} not GREEN in strict mode`)); break; } }
}
...
for (const [k, v] of Object.entries(state.blocks)) {
  const icon = v.status === 'GREEN' ? C.g('\u2705') : v.status === 'RED' || v.status === 'NO-GO' ? C.r('\u274C') : C.y('\uD83D\uDFE1');
  console.log(`  Block ${k}: ${icon} ${v.status} \u2014 ${v.note}`);
}
const fullyClosed = ['1','2','6sec','3','4','5','7'].every(k => state.blocks[k]?.status === 'GREEN');
console.log();
console.log(' Phase 7: ' + (fullyClosed ? C.g('FULLY CLOSED \u2705') : C.y('NOT YET CLOSED \u2014 see blocks above')));
console.log(C.c('='.repeat(70)));
