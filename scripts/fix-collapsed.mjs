...
function fixContent(content) {
  ...
  if (codeLinesAfter.length === 0) {
    ...
    const codeStart = /\b(export\s+(default\s+)?(class|function|interface|type|const|enum|abstract\s+class|async\s+function)|import\s+(type\s+)?\{|import\s+type\s+\*|abstract\s+class|class\s+\w+|function\s+\w+|const\s+\w+:\s*\w|type\s+\w+\s*=|interface\s+\w+)/;
