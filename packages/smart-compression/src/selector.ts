...
export function selectStrategy(conv: Conversation, cfg: SmartCompressionConfig): EngineDecision {
  if (cfg.selectionPolicy === "fixed") {
    return {
      strategyId: cfg.defaultStrategy,
      reason: `fixed policy -> ${cfg.defaultStrategy}`,
  ...
  if (ratio <= 1.2)
    return {
      strategyId: "drop",
      reason: `over by ${ratio.toFixed(2)}x -> drop`,
      overflowAccepted: false,
    };
  if (ratio <= 2.0)
    return {
      strategyId: "distill",
      reason: `over by ${ratio.toFixed(2)}x -> distill`,
      overflowAccepted: false,
    };
  return {
    strategyId: "compress-chain",
    reason: `over by ${ratio.toFixed(2)}x -> chain`,
    overflowAccepted: false,
  };
}
