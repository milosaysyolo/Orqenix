...
function denyVerifier(code = "E_CAP_EXPIRED"): StructuralCapabilityVerifier {
  return {
    async verify() {
      return { ok: false, code, message: "denied" };
    },
  };
}
function mkHooks() {
  ...
  return {
    hooks: {
      logger: new MeshLogger({ sink: buf.sink, level: "debug" }),
...
describe("makeInboundDispatch", () => {
  it("runs verify before handler and emits rpc.in/rpc.out", async () => {
    ...
    const handler = vi.fn(
      async (r: MeshRequest): Promise<MeshResponse> => ({ id: r.id, status: "ok" }),
    );
  ...
  it("serves cached response on dedup hit without re-running handler", async () => {
    const dedup = new CrossTransportDedup();
    const handler = vi.fn(
      async (r: MeshRequest): Promise<MeshResponse> => ({
        id: r.id,
        status: "ok",
