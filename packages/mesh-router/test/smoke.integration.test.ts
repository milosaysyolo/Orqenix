...
function makeFakeTransport(
  kind: string,
  behavior: (req: MeshRequest) => Promise<MeshResponse>,
): MeshTransport {
  let registered:
    | ((
        req: MeshRequest,
        ctx: { authenticatedScope: ScopeId; peerId?: string; remoteAddr?: string },
  ...
  return {
    ...
    async start() {},
    async stop() {},
    async send(_a: any, req: MeshRequest) {
      return behavior(req);
    },
    onRequest(h: any) {
      registered = h;
...
describe("Part 9 smoke: failover libp2p -> http with dedup integrity", () => {
  it("libp2p times out, http succeeds, handler invoked exactly once via dedup", async () => {
    ...
    const http = makeFakeTransport("http", async (req) => ({
      id: req.id,
      status: "ok",
    ...
    const okVerifier: StructuralCapabilityVerifier = {
      async verify() {
    ...
    const handlerCalls = vi.fn(
      async (req: MeshRequest): Promise<MeshResponse> => ({
        id: req.id,
        status: "ok",
        payload: new Uint8Array([0xee]),
      }),
