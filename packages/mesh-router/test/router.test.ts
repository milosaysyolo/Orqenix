...
function makeFakeTransport(kind: string, statuses: MeshStatus[], delays?: number[]): MeshTransport & { calls: number } {
  let i = 0;
  const t = {
    kind,
...
function makeRegistry(transports: MeshTransport[]): TransportRegistry {
  return {
    ...
    reachable(_id) { return transports.slice(); },
...
function addrFor(kind: string): MeshAddress {
  if (kind === 'http') return { kind: 'http', baseUrl: 'http://127.0.0.1:1' } as unknown as MeshAddress;
  if (kind === 'libp2p') return { kind: 'libp2p', multiaddr: '/ip4/127.0.0.1/tcp/1' } as unknown as MeshAddress;
