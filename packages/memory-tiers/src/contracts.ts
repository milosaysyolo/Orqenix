...
export const TierPromotionPolicySchema = z.object({
...
export const DEFAULT_POLICY: TierPromotionPolicy = TierPromotionPolicySchema.parse({
  workingToEpisodic: {}, episodicToSemantic: {}, semanticToProcedural: {},
});

export class InvalidTierError extends OrqenixError {
  constructor(value: unknown) { super(`invalid memory tier: ${String(value)}`, 'INVALID_TIER'); }
}
export class InvalidMemoryTypeError extends OrqenixError {
  constructor(value: unknown) { super(`invalid memory type: ${String(value)}`, 'INVALID_MEMORY_TYPE'); }
}
export class MemoryNotFoundError extends OrqenixError {
  constructor(id: string) { super(`memory not found: ${id}`, 'MEMORY_NOT_FOUND'); }
}
export class ImmutableMemoryError extends OrqenixError {
  constructor(id: string) { super(`memory is immutable (procedural tier): ${id}`, 'IMMUTABLE_MEMORY'); }
}
