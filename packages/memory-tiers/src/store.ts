...
import {
  ImmutableMemoryError, MemoryNotFoundError,
  type MemoryEntry, type MemoryId, type MemoryTier, type MemoryType,
} from './contracts.js';
...
interface Row {
  memory_id: string; tier: MemoryTier; type: MemoryType; content: string;
  content_hash: string; source_entry_ids: string; confidence: number;
  created_at: string; last_accessed_at: string; access_count: number;
  scope_id: string; metadata_json: string;
}

function toEntry(r: Row): MemoryEntry {
  return {
    id: r.memory_id as MemoryId,
    tier: r.tier, type: r.type, content: r.content,
    contentHash: r.content_hash,
    sourceEntryIds: JSON.parse(r.source_entry_ids) as string[],
    confidence: r.confidence,
    createdAt: r.created_at, lastAccessedAt: r.last_accessed_at,
    accessCount: r.access_count, scopeId: r.scope_id,
...
export class MemoryTierStore {
  ...
  insert(input: Omit<MemoryEntry, 'id' | 'contentHash' | 'createdAt' | 'lastAccessedAt' | 'accessCount'>): MemoryEntry {
    const contentHash = hashString(`${input.type}\n${input.content}`);
    const id = newMemoryId(contentHash, input.scopeId);
    const now = this.now();

    const existing = this.conn.prepare<Row>(
      `SELECT * FROM memory_entries WHERE content_hash = ? AND scope_id = ?`,
    ...
    this.conn.prepare(
      ...
      id, input.tier, input.type, input.content, contentHash,
      JSON.stringify(input.sourceEntryIds), input.confidence,
      now, now, input.scopeId, JSON.stringify(input.metadata ?? {}),
    );

    return {
      id, tier: input.tier, type: input.type, content: input.content,
      contentHash, sourceEntryIds: input.sourceEntryIds, confidence: input.confidence,
      createdAt: now, lastAccessedAt: now, accessCount: 0,
      scopeId: input.scopeId, metadata: input.metadata ?? {},
    };
  }

  getById(id: MemoryId): MemoryEntry {
    const r = this.conn.prepare<Row>(`SELECT * FROM memory_entries WHERE memory_id = ?`).get(id) as Row | undefined;
  ...
  countByTier(): Record<MemoryTier, number> {
    const rows = this.conn.prepare<{ tier: MemoryTier; c: number }>(
    ...
    const result: Record<MemoryTier, number> = { working: 0, episodic: 0, semantic: 0, procedural: 0 };
