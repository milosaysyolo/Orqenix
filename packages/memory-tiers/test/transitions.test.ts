...
function mk(overrides: Partial<MemoryEntry> = {}): MemoryEntry {
  return {
    ...
    tier: 'working', type: 'fact', content: 'x',
...
describe('transitions', () => {
  ...
  it('episodic -> semantic requires confidence', () => {
    expect(evaluatePromotion(mk({ tier: 'episodic', accessCount: 10, confidence: 0.5 }), NOW, DEFAULT_POLICY)).toBeNull();
    expect(evaluatePromotion(mk({ tier: 'episodic', accessCount: 10, confidence: 0.9 }), NOW, DEFAULT_POLICY)).toBe('semantic');
  });
  it('semantic -> procedural only for required types', () => {
    expect(evaluatePromotion(mk({ tier: 'semantic', type: 'fact', accessCount: 20 }), NOW, DEFAULT_POLICY)).toBeNull();
    expect(evaluatePromotion(mk({ tier: 'semantic', type: 'skill', accessCount: 20 }), NOW, DEFAULT_POLICY)).toBe('procedural');
  });
  it('procedural is terminal', () => {
    expect(evaluatePromotion(mk({ tier: 'procedural', type: 'skill', accessCount: 999 }), NOW, DEFAULT_POLICY)).toBeNull();
  });
