...
describe('MemoryTierStore', () => {
  ...
  it('inserts and retrieves by id', () => {
    const e = store.insert({
      tier: 'working', type: 'fact', content: 'A',
  ...
  it('insert is idempotent by content_hash + scope_id', () => {
    const a = store.insert({
      tier: 'working', type: 'fact', content: 'dup',
      sourceEntryIds: ['ce:1'], confidence: 0.8, scopeId: SCOPE, metadata: {},
    });
    const b = store.insert({
      tier: 'working', type: 'fact', content: 'dup',
      sourceEntryIds: ['ce:2'], confidence: 0.9, scopeId: SCOPE, metadata: {},
    });
  ...
  it('listByTier returns insertion order', () => {
    store.insert({ tier: 'working', type: 'fact', content: 'a', sourceEntryIds: ['ce:1'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
    store.insert({ tier: 'working', type: 'fact', content: 'b', sourceEntryIds: ['ce:2'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
    store.insert({ tier: 'working', type: 'fact', content: 'c', sourceEntryIds: ['ce:3'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
    expect(store.listByTier('working').map((e) => e.content)).toEqual(['a', 'b', 'c']);
  });

  it('recordAccess increments counter', () => {
    const e = store.insert({ tier: 'working', type: 'fact', content: 'x', sourceEntryIds: ['ce:1'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
  ...
  it('promote refuses procedural tier', () => {
    const e = store.insert({ tier: 'semantic', type: 'skill', content: 'q', sourceEntryIds: ['ce:1'], confidence: 0.9, scopeId: SCOPE, metadata: {} });
  ...
  it('countByTier returns full breakdown', () => {
    store.insert({ tier: 'working',  type: 'fact', content: 'a', sourceEntryIds: ['ce:1'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
    store.insert({ tier: 'episodic', type: 'fact', content: 'b', sourceEntryIds: ['ce:2'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
    store.insert({ tier: 'episodic', type: 'fact', content: 'c', sourceEntryIds: ['ce:3'], confidence: 0.8, scopeId: SCOPE, metadata: {} });
