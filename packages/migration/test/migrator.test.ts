...
import {
  PhaseFourToFiveMigrator, backupDatabase, restoreFromBackup, verifyBackup,
  MigrationError, BackupMissingError,
} from '../src';
...
describe('PhaseFourToFiveMigrator', () => {
  ...
  it('restoreFromBackup throws BackupMissingError when source missing', async () => {
    await expect(restoreFromBackup('/no/such/file.bak', join(dir, 'x.sqlite'))).rejects.toThrow(BackupMissingError);
  });
});
