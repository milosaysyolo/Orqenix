...
describe('Phase 6 migrations', () => {
  it('200: up creates mesh_transports, down drops it cleanly', async () => {
    ...
    try {
      ...
      m200.up(db);
      const has = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='mesh_transports'`).get();
      expect(has).toBeDefined();
      m200.down(db);
      const after = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='mesh_transports'`).get();
  ...
  it('201: up creates mesh_dedup_state, down drops it cleanly', async () => {
    ...
    try {
      ...
      db.pragma('journal_mode = WAL');
      m201.up(db);
      const has = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='mesh_dedup_state'`).get();
      expect(has).toBeDefined();
      m201.down(db);
      const after = db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='mesh_dedup_state'`).get();
      expect(after).toBeUndefined();
      db.close();
    } finally {
      await rm(dir, { recursive: true, force: true });
