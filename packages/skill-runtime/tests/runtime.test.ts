...
describe("SkillRuntime", () => {
  ...
  it("throws SkillNotFoundError for unregistered skill", async () => {
    await expect(runtime.invoke("@nope/skill", {}, { clientId: "test" })).rejects.toBeInstanceOf(
      SkillNotFoundError,
    );
  });
  ...
  it("invokes a registered skill", async () => {
    ...
    const result = await runtime.invoke("@example/skill", { x: 1 }, { clientId: "test" });
  ...
  it("rejects input missing required field", async () => {
    ...
    await expect(runtime.invoke("@example/skill2", {}, { clientId: "test" })).rejects.toThrow(
      /Missing required input field: x/,
    );
  });
