...
describe("Migrator", () => {
  ...
  it("apply backfills branch_id + memory_level on entries", async () => {
    ...
    const row = db
      .prepare("SELECT branch_id, memory_level FROM decision_entries WHERE id = ?")
      .get("d1") as { branch_id: string; memory_level: string } | undefined;
    db.close();
