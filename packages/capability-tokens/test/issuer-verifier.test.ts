...
describe("issuer + verifier integration", () => {
  let root: string;
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "orqenix-iv-"));
  });
  afterEach(async () => {
  ...
  it("rejects insufficient capability", async () => {
    ...
    await expect(verifier.verify(encoded, "write:kb-docs")).rejects.toThrow(
      InsufficientCapabilityError,
    );
  });
