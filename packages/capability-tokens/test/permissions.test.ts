...
function tok(caps: string[], maxDelegationDepth = 0): CapabilityToken {
  ...
  return {
    header: { alg: "EdDSA", typ: "ORQX", kid: payload.iss },
    payload,
...
describe("permissions", () => {
  ...
  it("scope pattern glob match", () => {
    expect(
      matchesCapability("write:kb-decisions:project-*", "write:kb-decisions:project-alpha"),
    ).toBe(true);
    expect(
      matchesCapability("write:kb-decisions:project-*", "write:kb-decisions:other-alpha"),
    ).toBe(false);
  });
  ...
  it("requireCapability throws InsufficientCapabilityError", () => {
    expect(() => requireCapability(tok(["read:kb-docs"]), "write:kb-docs")).toThrow(
      InsufficientCapabilityError,
    );
  });
