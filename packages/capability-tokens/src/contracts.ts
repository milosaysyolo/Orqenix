...
export const RESOURCES = ['kb-docs', 'kb-code', 'kb-chat', 'kb-decisions', 'audit-log', '*'] as const;
export type Resource = (typeof RESOURCES)[number];

export const CAPABILITY_PATTERN = /^(read|write|delegate|query|distill|mesh):([a-z0-9-]+|\*)(:[a-zA-Z0-9*_./-]+)?$/;
...
}).strict().refine((p) => p.exp > p.iat && p.nbf >= p.iat, {
...
export class TokenExpiredError extends OrqenixError {
  constructor(public readonly exp: number, public readonly now: number) {
    super(`token expired at ${new Date(exp * 1000).toISOString()}, now is ${new Date(now * 1000).toISOString()}`, 'TOKEN_EXPIRED');
  }
}
export class TokenNotYetValidError extends OrqenixError {
  constructor(public readonly nbf: number, public readonly now: number) {
    super(`token not valid until ${new Date(nbf * 1000).toISOString()}, now is ${new Date(now * 1000).toISOString()}`, 'TOKEN_NOT_YET_VALID');
  }
}
...
export class InsufficientCapabilityError extends OrqenixError {
  constructor(public readonly required: Capability, public readonly granted: readonly Capability[]) {
