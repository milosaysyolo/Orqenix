...
import {
  estimateTokens,
  type InjectionInput, type InjectionOutput, type InjectionStrategy,
} from './contracts.js';
...
export class StrategyA implements InjectionStrategy {
  ...
  apply(input: InjectionInput): InjectionOutput {
    ...
    const messages: ChatMessage[] = sysContent ? [{ role: 'system', content: sysContent }, ...rest] : rest;
    return {
      messages,
...
export class StrategyD implements InjectionStrategy {
  ...
  apply(input: InjectionInput): InjectionOutput {
    ...
    for (let i = input.messages.length - 1; i >= 0; i--) {
      if (input.messages[i]!.role === 'user') { lastUserIdx = i; break; }
    }
