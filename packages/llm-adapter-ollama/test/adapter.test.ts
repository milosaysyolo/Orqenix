...
describe('OllamaAdapter', () => {
  ...
  it('estimates tokens when Ollama omits counts', async () => {
    const a = new OllamaAdapter({
      fetchImpl: mockFetch(async () => new Response(JSON.stringify({
        message: { content: 'aaaaaaaa' }, done_reason: 'stop',
      }), { status: 200, headers: { 'content-type': 'application/json' } })),
    });
  ...
  it('maps non-200 to LlmProviderError', async () => {
    ...
    await expect(a.complete({ messages: [{ role: 'user', content: 'x' }] })).rejects.toThrow(LlmProviderError);
  });

  it('maps AbortError to LlmTimeoutError', async () => {
    const a = new OllamaAdapter({
      timeoutMs: 10,
      fetchImpl: mockFetch(() => new Promise<Response>((_, rej) => {
        setTimeout(() => rej(Object.assign(new Error('aborted'), { name: 'AbortError' })), 30);
      })),
    });
    await expect(a.complete({ messages: [{ role: 'user', content: 'x' }] })).rejects.toThrow(LlmTimeoutError);
  });

  it('isHealthy returns true on 200', async () => {
    const a = new OllamaAdapter({ fetchImpl: mockFetch(async () => new Response('{}', { status: 200 })) });
    expect(await a.isHealthy()).toBe(true);
  });

  it('isHealthy returns false on network error', async () => {
    const a = new OllamaAdapter({ fetchImpl: mockFetch(async () => { throw new Error('econnrefused'); }) });
