...
export class LlmProviderError extends OrqenixError {
  constructor(provider: string, reason: string) {
    super(`${provider} provider error: ${reason}`, "LLM_PROVIDER");
  }
}
export class LlmAuthError extends OrqenixError {
  constructor(provider: string) {
    super(`${provider} authentication failed (check API key)`, "LLM_AUTH");
  }
}
export class LlmRateLimitError extends OrqenixError {
  constructor(provider: string, retryAfterMs?: number) {
    super(
      `${provider} rate-limited` + (retryAfterMs ? ` retry after ${retryAfterMs}ms` : ""),
      "LLM_RATE_LIMIT",
    );
  }
}
