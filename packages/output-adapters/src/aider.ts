...
export const aiderOutputAdapter: OutputAdapter = {
  ...
  async serialize(csf: CanonicalSkillFormat): Promise<SerializedFormat> {
    ...
    if (preserved) {
      return { content: stringifyYaml(preserved, { indent: 2 }), suggestedPath: 'aider_plugin.yaml', format: 'yaml' };
    }
    const plugin = {
    ...
    return { content: stringifyYaml(plugin, { indent: 2 }), suggestedPath: 'aider_plugin.yaml', format: 'yaml' };
  },
