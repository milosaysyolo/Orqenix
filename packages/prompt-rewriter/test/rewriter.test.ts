...
function fakeAdapter(out: string): LlmAdapter {
  return {
    ...
    async complete(_req: LlmRequest): Promise<LlmResponse> {
      return { content: out, finishReason: 'stop', tokensIn: 1, tokensOut: 1, model: 'fake-1', provider: 'fake', latencyMs: 1 };
    },
...
describe('PromptRewriter', () => {
  ...
  beforeEach(async () => {
    ...
    store.insert({ tier: 'episodic', type: 'preference', content: 'I prefer Rust', sourceEntryIds: ['ce:1'], confidence: 0.9, scopeId: SCOPE, metadata: {} });
    store.insert({ tier: 'episodic', type: 'decision',   content: 'I decided to use SQLite', sourceEntryIds: ['ce:2'], confidence: 0.85, scopeId: SCOPE, metadata: {} });
  ...
  it('setStrategy switches at runtime', async () => {
    ...
    const out = await rewriter.rewrite({ messages: [{ role: 'user', content: 'What do I prefer?' }] });
  ...
  it('does not call LLM rewrite unless enabled', async () => {
    ...
    const adapter: LlmAdapter = {
      provider: 'count', model: 'm',
      async complete() { called++; return { content: 'x', finishReason: 'stop', tokensIn: 0, tokensOut: 0, model: 'm', provider: 'count', latencyMs: 0 }; },
      async isHealthy() { return true; },
    };
  ...
  it('LLM rewrite consolidates the system message when enabled', async () => {
    ...
    const rw = new PromptRewriter({ recall: new KeywordRecall(store, SCOPE), adapter, enableRewriteFn: true });
  ...
  it('gracefully degrades when adapter throws', async () => {
    ...
    const rw = new PromptRewriter({ recall: new KeywordRecall(store, SCOPE), adapter, enableRewriteFn: true });
    const out = await rw.rewrite({ messages: [{ role: 'system', content: 'base' }, { role: 'user', content: 'rust?' }] });
