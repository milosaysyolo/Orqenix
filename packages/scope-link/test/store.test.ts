...
describe('ScopeLinkStore', () => {
  ...
  it('rejects duplicate (local, remote, direction)', () => {
    ...
    expect(() => store.create({ remoteScopeId: B, direction: 'outbound' })).toThrow(LinkAlreadyExistsError);
  });
  ...
  it('updateStatus accepts new capabilityTokenJti', () => {
    store.create({ remoteScopeId: B, direction: 'outbound' });
    const updated = store.updateStatus(B, 'outbound', 'active', { tokenJti: 'tok:DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD' });
