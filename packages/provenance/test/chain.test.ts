...
import {
  buildChain, verifyChain, appendTag, rootTag, computeChainHash,
...
describe('provenance chain', () => {
  ...
  it('buildChain rejects root with parentChainHash', () => {
    const bogus: ProvenanceTag = {
      ...tag(), parentChainHash: '0'.repeat(64),
    };
  ...
  it('mesh source without tokenJti is rejected', () => {
    expect(() => buildChain([tag({ sourceKind: 'mesh' }) as ProvenanceTag])).toThrow(InvalidProvenanceTagError);
  });
  ...
  it('rejects chain longer than 32 tags', () => {
    ...
    expect(() => appendTag(chain, tag({ sourceScopeId: A, sourceKind: 'distilled' }))).toThrow(InvalidProvenanceTagError);
  });

  it('computeChainHash is deterministic + order-sensitive', () => {
    const t1 = { ...tag({ sourceScopeId: A }) } as ProvenanceTag;
    const t2 = { ...tag({ sourceScopeId: B, sourceKind: 'mesh', tokenJti: TOK }), parentChainHash: computeChainHash([t1]) } as ProvenanceTag;
