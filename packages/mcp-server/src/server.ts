...
/** Maps a tool name to its required permission */  private toolPermission(name: string): string | null {    const map: Record<string, string> = {      orqenix_recall_memory: 'memory.read:chat',      orqenix_record_decision: 'memory.write:decision',      orqenix_record_lesson: 'memory.write:lesson',      orqenix_query_codekb: 'memory.read:code',      orqenix_invoke_skill: 'skill.invoke',      orqenix_link_scope: 'scope.write',      orqenix_verify_audit_chain: 'audit.read',      orqenix_promote_to_branch: 'memory.write:decision',      orqenix_report_session_start: 'scope.read',      orqenix_report_session_resume: 'scope.read',    };    return map[name] ?? null;  }}