...
// @orqenix/mcp-server , CLI entry (orqenix-mcp)
//
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--client-id') { const v = argv[++i]; if (v) args.clientId = v; }
  }
  if (!args.project) {
...
async function main(): Promise<void> {
  ...
  switch (args.transport) {
    ...
    default:
      log(`Unknown transport: ${args.transport}`);
      process.exit(1);
  }
}
...
void main().catch((e) => {
  process.stderr.write(`[orqenix-mcp] FATAL: ${(e as Error).message}\n`);
  process.exit(1);
});
