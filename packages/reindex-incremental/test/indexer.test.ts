...
describe('ReindexIndexer + Reindexer', () => {
  ...
  function entry(relPath: string, hash = '0'.repeat(64)): IndexEntry {
    return { relPath, scopeId: SCOPE, contentHash: hash, sizeBytes: 10, modifiedAt: new Date().toISOString() };
  }
  ...
  it('applyEvents handles add/change/unlink', async () => {
    ...
    const r1 = await rx.applyEvents([{ kind: 'add', path: join(rootDir, 'a.txt'), relPath: 'a.txt', timestamp: new Date().toISOString() }]);
    expect(r1.filesAdded).toBe(1);
    await writeFile(join(rootDir, 'a.txt'), 'v2');
    const r2 = await rx.applyEvents([{ kind: 'change', path: join(rootDir, 'a.txt'), relPath: 'a.txt', timestamp: new Date().toISOString() }]);
    expect(r2.filesUpdated).toBe(1);
    await unlink(join(rootDir, 'a.txt'));
    const r3 = await rx.applyEvents([{ kind: 'unlink', path: join(rootDir, 'a.txt'), relPath: 'a.txt', timestamp: new Date().toISOString() }]);
  ...
  it('applyEvents ignores .git paths', async () => {
    ...
    const r = await rx.applyEvents([{
      kind: 'add', path: join(rootDir, '.git', 'HEAD'),
      relPath: '.git/HEAD', timestamp: new Date().toISOString(),
    }]);
