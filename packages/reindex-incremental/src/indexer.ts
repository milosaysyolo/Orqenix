...
const DEFAULT_IGNORE_RX =
  /(^|\/)(\.git|node_modules|dist|\.orqenix\/identity\.key|\.orqenix\/gate-reports)(\/|$)/;
...
export class ReindexIndexer {
  ...
  remove(scopeId: string, relPath: string): boolean {
    const r = this.conn
      .prepare(`DELETE FROM reindex_entries WHERE scope_id = ? AND rel_path = ?`)
      .run(scopeId, relPath);
    return r.changes > 0;
  }
  ...
  count(scopeId: string): number {
    const r = this.conn
      .prepare<{ c: number }>(`SELECT COUNT(*) as c FROM reindex_entries WHERE scope_id = ?`)
  ...
  async applyEvents(events: FileEvent[]): Promise<ReindexStats> {
    ...
    for (const e of events) {
      ...
      try {
        ...
        stats.filesRemoved += this.indexer.remove(this.scopeId, e.relPath) ? 1 : 0;
        continue;
      }
      if (s.size > MAX_FILE_BYTES) continue;
      let bytes: Buffer;
      try {
        bytes = await readFile(e.path);
      } catch {
        continue;
      }
      const contentHash = hashBytes(bytes) as ContentHash;
