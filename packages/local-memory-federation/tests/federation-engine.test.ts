...
describe('FederationEngine (E2E)', () => {
  ...
  it('queries other project when approval exists', async () => {
    ...
    const queryEvent = events.find(
      (e) => e.kind === 'memory.cross_project_query'
    );
  ...
  it('validates query input via Zod', async () => {
    ...
    await expect(
      engine.crossProjectQuery({ query: '', limit: 10 })
    ).rejects.toThrow();

    await expect(
      engine.crossProjectQuery({ query: 'ok', limit: -1 })
    ).rejects.toThrow();

    await expect(
      engine.crossProjectQuery({ query: 'ok', limit: 500 })
    ).rejects.toThrow();
  });
