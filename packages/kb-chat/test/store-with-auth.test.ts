...
import { ChatStore, CHAT_KB_MIGRATIONS, createChatVecTable, ChatWriteUnauthorizedError } from '../src';
...
describe('ChatStore (verifier-gated writes)', () => {
  ...
  it('accepts write with valid write:kb-chat token', async () => {
    ...
    const verifier = new TokenVerifier({ getIssuerPublicKey: async (kid) => (kid === iss ? kp.publicKey : null) });
