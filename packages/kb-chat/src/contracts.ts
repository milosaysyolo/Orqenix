...
export const AppendChatEntryInputSchema = z.object({
...
export class SessionNotFoundError extends OrqenixError {
  constructor(id: string) { super(`chat session not found: ${id}`, 'SESSION_NOT_FOUND'); }
}
export class EntryNotFoundError extends OrqenixError {
  constructor(id: string) { super(`chat entry not found: ${id}`, 'ENTRY_NOT_FOUND'); }
}
export class ChatWriteUnauthorizedError extends OrqenixError {
  constructor(reason: string) { super(`chat write unauthorized: ${reason}`, 'CHAT_WRITE_UNAUTH'); }
}
export class HashChainBrokenError extends OrqenixError {
  constructor(expected: string, actual: string) {
    super(`chat entry hash chain broken: expected prev ${expected.slice(0, 12)}..., got ${actual.slice(0, 12)}...`, 'HASH_CHAIN_BROKEN');
  }
}
