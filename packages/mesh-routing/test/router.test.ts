...
describe("MeshRouter", () => {
  ...
  it("respects k cap on aggregated hits", async () => {
    activateLink(B);
    transport.setHandler(B, async () =>
      Array.from({ length: 20 }, (_, i) => hit(B, `t${i}`, 1 - i * 0.01)),
    );
  ...
  it("records timeout outcome per-target", async () => {
    ...
    transport.setHandler(
      B,
      async () =>
        new Promise((resolve) => setTimeout(() => resolve([hit(B, "late", 1)]), 500)) as Promise<
          MeshQueryHit[]
        >,
    );
  ...
  it("accepts a 2-tag provenance chain (mesh hop)", async () => {
    ...
    const meshHop = appendTag(root, {
      sourceScopeId: A,
      producedAt: "2026-06-02T00:00:01Z",
      sourceKind: "mesh",
      tokenJti: "tok:DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD",
    });
    transport.setHandler(B, async () => [
      { scopeId: B, text: "forwarded", score: 0.7, provenance: meshHop },
    ]);
    const r = await router.query({ text: "q", k: 5, timeoutMs: 1000 });
    expect(r.hits).toHaveLength(1);
  ...
  describe("suggestLinks", () => {
    it("suggests frequent-failure for scopes failing >= 60% over >= 5 runs", () => {
      const history = Array.from({ length: 5 }, () => ({
        ...
        hits: [],
        outcomes: [
          { scopeId: B, ok: false, reason: "timeout", message: "t", durationMs: 100 } as any,
        ],
        totalDurationMs: 100,
        quorumReached: false,
      }));
        ...
        outcomes: [
          {
            scopeId: B,
            ok: true,
            hits: [hit(B, "a", 0.95), hit(B, "b", 0.9)],
            durationMs: 10,
          } as any,
          { scopeId: C, ok: true, hits: [hit(C, "a", 0.3)], durationMs: 10 } as any,
        ],
        totalDurationMs: 10,
        quorumReached: true,
      }));
