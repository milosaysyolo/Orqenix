...
describe("CapabilityVerifier p95 benchmark", () => {
  RUN(
    `p95 verify under 10ms (CI tolerance +${CI_TOLERANCE_MS}ms) over ${ITER} iterations`,
    async () => {
      ...
      console.log(
        `[bench] verify p50=${p50.toFixed(3)}ms p95=${p95.toFixed(3)}ms p99=${p99.toFixed(3)}ms n=${ITER}`,
      );
