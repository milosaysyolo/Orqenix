...
describe('Ed25519IdentityVerifier (structural)', () => {
  it('verifies a signer-produced proof end-to-end', async () => {
    ...
    expect(await verifier.verifyScopeSig(fromScope, 'rid-X', 'scp_b3_B' as ScopeId, sig)).toBe(true);
  });
  ...
  it('rejects wrong scope', async () => {
    ...
    expect(await verifier.verifyScopeSig(fromScope, 'rid-X', 'scp_b3_OTHER' as ScopeId, sig)).toBe(false);
  });
  ...
  it('rejects when issuer key is not in the store', async () => {
    ...
    expect(await verifier.verifyScopeSig('scp_b3_A' as ScopeId, 'rid-X', 'scp_b3_B' as ScopeId, sig)).toBe(false);
  });
