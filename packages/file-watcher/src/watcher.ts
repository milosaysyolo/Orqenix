...
import {
  WatcherConfigSchema, WatcherError,
  type FileEvent, type FileEventBatchListener, type FileEventKind, type WatcherConfig,
} from './contracts.js';
...
export class FileWatcher {
  ...
  async start(listener: FileEventBatchListener): Promise<void> {
    ...
    await new Promise<void>((res) => {
      this.watcher?.once('ready', () => { this.status = 'running'; res(); });
    });
  }
  ...
  private enqueue(kind: FileEventKind, p: string): void {
    ...
    const evt: FileEvent = {
      kind, path: abs, relPath: rel,
      timestamp: new Date().toISOString(),
    };
    this.pending.push(evt);
    if (this.debounceTimer) clearTimeout(this.debounceTimer);
    this.debounceTimer = setTimeout(() => { void this.flush(); }, this.cfg.debounceMs);
  }
  ...
  private async flush(): Promise<void> {
    ...
    try { await this.listener(batch); }
    catch { /* listener errors are caller's problem */ }
  }
