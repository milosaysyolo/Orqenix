...
describe("Part 4 smoke: TCP + WebSockets concurrent", () => {
  it("A (tcp+ws) -> B (tcp+ws) ok over both transports, clean teardown", async () => {
    ...
    const tcpResp = await A.send(
      { kind: "libp2p", multiaddr: tcpAddr! } as MeshAddress,
      mkReq("01HV0R6X3M8YQ9G7F2D5W1TCP55"),
    );
    const wsResp = await A.send(
      { kind: "libp2p", multiaddr: wsAddr! } as MeshAddress,
      mkReq("01HV0R6X3M8YQ9G7F2D5W1WSS55"),
    );
