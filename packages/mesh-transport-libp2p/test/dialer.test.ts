...
describe("Dialer", () => {
  it("succeeds on first attempt", async () => {
    ...
    const d = new Dialer({
      backoff: { maxAttempts: 3, baseDelayMs: 1, rand: () => 0.5, sleep: async () => {} },
    });
    const conn = await d.dial(node as unknown as Parameters<typeof d.dial>[0], MA, {
  ...
  it("retries up to maxAttempts then throws", async () => {
    ...
    await expect(
      d.dial(node as unknown as Parameters<typeof d.dial>[0], MA, {
        deadlineMs: Date.now() + 1000,
      }),
    ).rejects.toThrow();
  ...
  it("stops at deadline", async () => {
    ...
    const d = new Dialer({
      backoff: { maxAttempts: 99, baseDelayMs: 1, rand: () => 0.5, sleep: async () => {} },
    });
    await expect(
  ...
  it("coalesces concurrent dials to the same peer", async () => {
    const node = fakeNodeOk();
    const d = new Dialer({
      backoff: { maxAttempts: 1, baseDelayMs: 1, rand: () => 0.5, sleep: async () => {} },
    });
