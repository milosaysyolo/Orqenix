    ...
    for (const row of rows) {
      ...
      for (const c of candidates) {
        ...
        const entry = this.memStore.insert({
          ...
          metadata: { matchedPattern: c.matchedPattern ?? null },
        } as Omit<
          MemoryEntry,
          "id" | "contentHash" | "createdAt" | "lastAccessedAt" | "accessCount"
        >);
        const afterCount = this.memStore.countByTier();
        const totalBefore =
          beforeCount.working +
          beforeCount.episodic +
          beforeCount.semantic +
          beforeCount.procedural;
        const totalAfter =
          afterCount.working + afterCount.episodic + afterCount.semantic + afterCount.procedural;
