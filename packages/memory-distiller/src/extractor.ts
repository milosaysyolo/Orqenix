...
const PATTERNS: Pattern[] = [
  ...
  { type: 'decision',   rx: /\b(decided\s+to|chose\s+to|going\s+with|we'?ll\s+use)\b/i,      confidence: 0.85 },
  ...
  { type: 'skill',      rx: /\b(how\s+to|the\s+way\s+to|technique\s+for|trick\s+for)\b/i,    confidence: 0.7  },
  { type: 'relationship', rx: /\b(\w+\s+(reports\s+to|works\s+with|manages|leads))\b/i,      confidence: 0.7  },
