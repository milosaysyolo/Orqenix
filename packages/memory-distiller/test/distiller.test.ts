...
describe('HeuristicDistiller', () => {
  ...
  it('is idempotent across reruns (watermark advances)', async () => {
    const s = chat.createSession({ scopeId: SCOPE, title: 't' });
    await chat.appendEntry({ sessionId: s.sessionId, role: 'user', content: 'I prefer SQLite for OSS', metadata: {} });
  ...
  it('skips system role entries', async () => {
    const s = chat.createSession({ scopeId: SCOPE, title: 't' });
    await chat.appendEntry({ sessionId: s.sessionId, role: 'system', content: 'I prefer Rust', metadata: {} });
    const stats = distiller.distillBatch();
  ...
  it('dedupes identical content across entries (content_hash idempotency)', async () => {
    const s = chat.createSession({ scopeId: SCOPE, title: 't' });
    await chat.appendEntry({ sessionId: s.sessionId, role: 'user', content: 'I prefer Rust', metadata: {} });
  ...
  it('distillAll processes everything to idle', async () => {
    const s = chat.createSession({ scopeId: SCOPE, title: 't' });
    for (let i = 0; i < 5; i++) {
      await chat.appendEntry({ sessionId: s.sessionId, role: 'user', content: `I decided to use lib-${i}`, metadata: {} });
    }
