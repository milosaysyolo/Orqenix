...
describe('DistillerWorker', () => {
  ...
  beforeEach(async () => {
    ...
    distiller = new HeuristicDistiller({ memStore, chatConn: conn, scopeId: SCOPE, config: { batchSize: 2 } });
  });
  ...
  it('processes all entries then stops on idle', async () => {
    ...
    for (let i = 0; i < 6; i++) {
      await chat.appendEntry({ sessionId: s.sessionId, role: 'user', content: `I prefer pattern-${i}`, metadata: {} });
    }
  ...
  it('stop() halts the loop', async () => {
    const s = chat.createSession({ scopeId: SCOPE, title: 't' });
    for (let i = 0; i < 50; i++) {
      await chat.appendEntry({ sessionId: s.sessionId, role: 'user', content: `I prefer x-${i}`, metadata: {} });
    }
