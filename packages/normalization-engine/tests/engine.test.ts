...
describe('NormalizationEngine', () => {
  ...
  it('throws NoAdapterMatchError when nothing matches', async () => {
    ...
    await expect(engine.import({ content: 'y' })).rejects.toBeInstanceOf(
      NoAdapterMatchError
    );
  });
  ...
  it('throws AmbiguousMatchError when top two tie', async () => {
    ...
    await expect(engine.import({ content: 'z' })).rejects.toBeInstanceOf(
      AmbiguousMatchError
    );
  });
  ...
  it('throws UnsupportedTargetError for unknown target', async () => {
    ...
    await expect(engine.export(csf, 'nonexistent')).rejects.toBeInstanceOf(
      UnsupportedTargetError
    );
  });
