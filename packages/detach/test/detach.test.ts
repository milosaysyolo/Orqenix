...
describe('Detach (planner + executor)', () => {
  ...
  it('execute rejects mismatched confirmation token', async () => {
    ...
    linkStore.updateStatus(B, 'outbound', 'active');
    const plan = planner.planUnlink(B);
    await expect(executor.execute(plan, 'detach:ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ')).rejects.toThrow(InvalidConfirmationError);
  ...
  it('execute rejects plan from a different localScopeId', async () => {
    ...
    await expect(executor.execute(tamperedPlan, plan.confirmationToken)).rejects.toThrow(InvalidConfirmationError);
  });
  ...
  it('replay attack: cannot reuse token for a different plan', async () => {
    ...
    await expect(executor.execute(p2, p1.confirmationToken)).rejects.toThrow(InvalidConfirmationError);
  });
});
