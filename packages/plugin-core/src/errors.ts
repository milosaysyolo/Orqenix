...
export class PluginNotFoundError extends PluginError {
  constructor(packagePath: string) {
    super("PLUGIN_NOT_FOUND", `Plugin not found at ${packagePath}. Check installation or path.`);
...
export class PluginInstallFailedError extends PluginError {
  constructor(packageName: string, reason: string, cause?: unknown) {
    super("PLUGIN_INSTALL_FAILED", `Plugin install failed for ${packageName}: ${reason}`, cause);
...
export class PluginActivateFailedError extends PluginError {
  constructor(packageName: string, reason: string, cause?: unknown) {
    super("PLUGIN_ACTIVATE_FAILED", `Plugin activate failed for ${packageName}: ${reason}`, cause);
...
export class PluginTimeoutError extends PluginError {
  constructor(packageName: string, timeoutMs: number) {
    super("PLUGIN_TIMEOUT", `Plugin ${packageName} exceeded wall-time limit of ${timeoutMs}ms`);
...
export class PluginNotRegisteredError extends PluginError {
  constructor(packageName: string) {
    super("PLUGIN_NOT_REGISTERED", `Plugin ${packageName} is not registered.`);
...
export class PluginDeniedError extends PluginError {
  constructor(packageName: string, reason: string) {
    super("PLUGIN_DENIED", `Plugin ${packageName} denied: ${reason}`);
