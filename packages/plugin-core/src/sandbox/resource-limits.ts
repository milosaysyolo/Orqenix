...
import type { SandboxConfig } from '../csf-schema';

/** Default resource limits (configurable via Workbench Settings) */
export const DEFAULT_RESOURCE_LIMITS: Required<
  Omit<SandboxConfig, 'fsReadPaths' | 'fsWritePaths'>
> & {
  fsReadPaths: string[];
  fsWritePaths: string[];
} = {
  cpuLimitPct: 25, // Plugin can use up to 25% of one core
  memoryLimitMb: 512, // Plugin can use up to 512MB RAM
  wallTimeLimitSec: 300, // 5 min hard timeout per invocation
  fsReadPaths: [], // Whitelist of readable paths
  fsWritePaths: [], // Whitelist of writable paths
  networkAllowed: false, // No network unless explicit
};
...
/**
 * Resolves effective resource limits by merging plugin's sandboxOverrides
 * with operator defaults. Operator defaults act as a CEILING: a plugin cannot
...
export function resolveResourceLimits(
  ...
  // Plugin can request LESS than operator allows, but NOT more (ceiling enforcement)
  return {
    cpuLimitPct:
      pluginOverrides.cpuLimitPct !== undefined
        ? Math.min(pluginOverrides.cpuLimitPct, base.cpuLimitPct)
        : base.cpuLimitPct,
    memoryLimitMb:
      pluginOverrides.memoryLimitMb !== undefined
        ? Math.min(pluginOverrides.memoryLimitMb, base.memoryLimitMb)
        : base.memoryLimitMb,
    wallTimeLimitSec:
      pluginOverrides.wallTimeLimitSec !== undefined
        ? Math.min(pluginOverrides.wallTimeLimitSec, base.wallTimeLimitSec)
        : base.wallTimeLimitSec,
    // FS paths: plugin's requested paths must be SUBSETS of operator-allowed paths
    ...
    networkAllowed:
      (pluginOverrides.networkAllowed ?? false) && base.networkAllowed,
  };
}

/**
 * Returns paths from `requested` that are within `allowed` (prefix match).
 * If `allowed` is empty, returns empty (deny-by-default).
 */
...
/**
 * Builds Node.js child_process spawn options from resolved limits.
 * Uses --max-old-space-size for memory, env vars for path whitelists.
 */
export function buildSpawnEnv(
  ...
  return {
    // Inherit minimal env; plugin shouldn't see host secrets
    PATH: process.env.PATH,
    NODE_ENV: 'production',
    // Orqenix-specific sandbox env
    ORQENIX_SANDBOX: '1',
