...
> & { fsReadPaths: string[]; fsWritePaths: string[] } = {
  cpuLimitPct: 25,
  memoryLimitMb: 512,
...
export function resolveResourceLimits(
  ...
  const base: ResolvedResourceLimits = {
    ...
    memoryLimitMb: (operatorDefaults.memoryLimitMb ??
      DEFAULT_RESOURCE_LIMITS.memoryLimitMb) as number,
    wallTimeLimitSec: (operatorDefaults.wallTimeLimitSec ??
      DEFAULT_RESOURCE_LIMITS.wallTimeLimitSec) as number,
    fsReadPaths: operatorDefaults.fsReadPaths ?? DEFAULT_RESOURCE_LIMITS.fsReadPaths,
    fsWritePaths: operatorDefaults.fsWritePaths ?? DEFAULT_RESOURCE_LIMITS.fsWritePaths,
    networkAllowed: (operatorDefaults.networkAllowed ??
      DEFAULT_RESOURCE_LIMITS.networkAllowed) as boolean,
  };
  if (!pluginOverrides) {
    return base;
  ...
  return {
    ...
    wallTimeLimitSec:
      pluginOverrides.wallTimeLimitSec !== undefined
        ...
        : base.wallTimeLimitSec, // FS paths: plugin's requested paths must be SUBSETS of operator-allowed paths
    fsReadPaths: intersectPaths(pluginOverrides.fsReadPaths ?? [], base.fsReadPaths),
    fsWritePaths: intersectPaths(pluginOverrides.fsWritePaths ?? [], base.fsWritePaths),
...
function intersectPaths(requested: string[], allowed: string[]): string[] {
  ...
  return requested.filter((req) => allowed.some((a) => req === a || req.startsWith(a + "/")));
}
