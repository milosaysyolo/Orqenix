...
export const embeddingModelHandler: PluginKindHandler<'embedding-model'> = {
  ...
  description:
    'Provides embedding generation for text/code (BGE, OpenAI, Voyage, jina, custom).',
  ...
  validateManifest(csf: CanonicalSkillFormat): ValidationResult {
    ...
    const isLocal =
      ...
      csf.manifest.keywords.includes('local');
    if (!isLocal && !csf.manifest.permissions.some((p) => p.startsWith('network.fetch'))) {
      warnings.push(
        "Remote embedding-model plugins (BYOK) require 'network.fetch' permission"
      );
    }
