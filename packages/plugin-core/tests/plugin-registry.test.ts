...
import { PluginAlreadyRegisteredError, PluginNotRegisteredError } from "../src/errors";
...
describe("PluginRegistry", () => {
  ...
  it("rejects duplicate registration", async () => {
    ...
    await expect(registry.register(makeDiscovery("@a/skill"))).rejects.toBeInstanceOf(
  ...
  it("throws when unregistering unknown plugin", async () => {
    await expect(registry.unregister("@nope/x")).rejects.toBeInstanceOf(PluginNotRegisteredError);
  });
