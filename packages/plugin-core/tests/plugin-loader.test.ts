...
async function writePlugin(dir: string, pkg: Record<string, unknown>): Promise<string> {
...
describe("PluginLoader", () => {
  ...
  it("loadAll discovers plugins and skips non-plugins", async () => {
    ...
    await writeFile(join(p2, "package.json"), JSON.stringify({ name: "plain", version: "1.0.0" }));
