...
describe('Resource Limits', () => {
  ...
  it('network only allowed if both plugin requests AND operator permits', () => {
    expect(
      resolveResourceLimits({ networkAllowed: true }, { networkAllowed: false })
        .networkAllowed
    ).toBe(false);
    expect(
      resolveResourceLimits({ networkAllowed: true }, { networkAllowed: true })
        .networkAllowed
    ).toBe(true);
  });
