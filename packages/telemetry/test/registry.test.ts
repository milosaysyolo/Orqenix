...
describe('MetricsRegistry', () => {
  ...
  it('sink receives every observation', () => {
    ...
    const sink: MetricSink = {
      onCounter(name, value) { observed.push({ kind: 'c', name, value }); },
      onGauge(name, value) { observed.push({ kind: 'g', name, value }); },
      onHistogram(name, value) { observed.push({ kind: 'h', name, value }); },
    };
    r.setSink(sink);
    r.counter('a').inc(2);
    r.gauge('b').set(9);
  ...
  it('reset clears all metrics', () => {
    const r = new MetricsRegistry();
    r.counter('a').inc(); r.gauge('b').set(1); r.histogram('c').observe(1);
    r.reset();
    const s = r.snapshot();
