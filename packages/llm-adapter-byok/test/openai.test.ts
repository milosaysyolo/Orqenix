...
describe('OpenAiAdapter', () => {
  it('completes with usage', async () => {
    const a = new OpenAiAdapter({
      ...
      fetchImpl: mockFetch(async () => new Response(JSON.stringify({
        ...
        usage: { prompt_tokens: 10, completion_tokens: 2 }, model: 'gpt-4o-mini',
  ...
  it('maps 401 to LlmAuthError', async () => {
    ...
    await expect(a.complete({ messages: [{ role: 'user', content: 'x' }] })).rejects.toThrow(LlmAuthError);
  });

  it('maps 429 to LlmRateLimitError', async () => {
    const a = new OpenAiAdapter({
      apiKey: 'k',
      fetchImpl: mockFetch(async () => new Response('{}', { status: 429, headers: { 'retry-after': '5' } })),
    });
    await expect(a.complete({ messages: [{ role: 'user', content: 'x' }] })).rejects.toThrow(LlmRateLimitError);
  });
