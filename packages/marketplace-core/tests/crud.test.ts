...
describe('MarketplaceCrud', () => {
  ...
  it('updates with version bump', async () => {
    await crud.create({
      ...
      permissions: [],
      external_agent_compat: ['claude-code'],
      tool: { name: 'upd', description: 'x', inputSchema: { type: 'object' }, outputSchema: { type: 'object' } },
    });
  ...
  it('bumps major correctly', async () => {
    await crud.create({
      ...
      permissions: [],
      external_agent_compat: ['claude-code'],
      tool: { name: 'maj', description: 'x', inputSchema: { type: 'object' }, outputSchema: { type: 'object' } },
    });
  ...
  it('rejects delete with wrong confirmation', async () => {
    ...
    await expect(
      crud.delete({ name: '@local/del2', confirmation: 'wrong' })
