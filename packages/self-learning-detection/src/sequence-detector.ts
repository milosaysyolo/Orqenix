...
import { type ActionSequence, type DetectionThresholds, DEFAULT_THRESHOLDS } from "./types";
...
export class SequenceDetector {
  ...
  extractSequences(events: ObservationEvent[]): ActionSequence[] {
    ...
    for (const sessionEvents of bySession.values()) {
      ...
      for (
        let len = this.thresholds.minSequenceLength;
        len <= this.thresholds.maxSequenceLength;
        len++
      ) {
        for (let start = 0; start + len <= sorted.length; start++) {
          ...
          const durationMs = window.reduce((sum, e) => sum + (e.outcome_duration_ms ?? 0), 0);
