...
function memoryContract(
  overrides: Partial<ModuleSettingsContract> = {}
...
describe('SettingsRegistry', () => {
  ...
  it('registers a module and audits it', async () => {
    ...
    expect(
      audit.getEvents().some((e) => e.kind === 'settings.module_registered')
    ).toBe(true);
  });

  it('throws when accessing unregistered module (Anti-pattern 37)', async () => {
    await expect(
      registry.resolve('@nope/module', 'foo', {})
  ...
  it('updates a setting at project level + audits', async () => {
    ...
    const changeEvents = audit
      .getEvents()
      .filter((e) => e.kind === 'settings.changed');
  ...
  it('fires hot reload handler on update', async () => {
    ...
    await registry.update(
      ...
      'hierarchy.level_boost.session',
      9.9,
      { level: 'project', hierarchyId: 'blake3:proj' }
    );
  ...
  it('rolls back on hot reload failure', async () => {
    ...
    // Audited the failure
    expect(
      audit.getEvents().some((e) => e.kind === 'settings.hot_reload_failed')
    ).toBe(true);
  });
