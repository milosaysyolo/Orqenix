...
describe("Export / Import", () => {
  ...
  it("replace mode clears existing overrides first", async () => {
    ...
    await registry.update("@orqenix/memory-engine", "hierarchy.level_boost.session", 9.0, {
      level: "user",
  ...
  it("rejects malformed import data", async () => {
    await expect(importSettings(registry, '{"version": 99}', { mode: "merge" })).rejects.toThrow();
  });
