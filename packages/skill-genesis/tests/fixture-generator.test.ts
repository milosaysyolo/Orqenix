...
function ev(session: string, action: string, payload: Record<string, unknown>, ts: string, outcome: 'success' | 'error' | null): ObservationEvent {
  return {
    id: `e-${session}-${action}`, timestamp: ts,
    project_id: 'p', branch_id: 'b', session_id: session, parent_session_id: null,
    agent_platform: 'cc', actor_kind: 'agent', actor_id: 'x',
    action_kind: action, action_payload: payload,
    outcome_kind: outcome, outcome_duration_ms: outcome ? 1000 : null, outcome_payload: null,
...
describe('FixtureGenerator', () => {
  ...
  it('caps fixtures at 10', () => {
    ...
    const fixtures = gen.generate(events, [{ name: 'x', type: 'number', variable: true, samples: [], required: true }]);
