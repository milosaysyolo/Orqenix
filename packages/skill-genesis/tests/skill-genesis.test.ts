...
function seedCandidateWithObservations(db: DB, observer: Observer): string {
  ...
  for (let i = 0; i < 3; i++) {
    const e1 = observer.capture({
      projectId: PROJECT, sessionId: `s${i}`, actorKind: 'agent', actorId: 'x',
      actionKind: 'file_edit', actionPayload: { file: `src/a${i}.ts` },
    });
    const e2 = observer.capture({
      projectId: PROJECT, sessionId: `s${i}`, actorKind: 'agent', actorId: 'x',
      actionKind: 'shell_command', actionPayload: { command: `npm test a${i}` },
      outcomeKind: 'success', outcomeDurationMs: 5000,
    });
  ...
  db.prepare(
    ...
    candId, PROJECT, '@local/edit-then-test', 'Edit then test workflow',
...
describe('SkillGenesis', () => {
  ...
  it('tags derived_from_observations + unverified (Anti-38)', async () => {
    ...
    const row = db.prepare('SELECT csf_json FROM local_plugins WHERE name = ?').get(result.skillName) as
      | { csf_json: string }
      | undefined;
