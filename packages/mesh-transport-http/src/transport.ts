...
export class HttpMeshTransport implements MeshTransport {
  ...
  private async handleInbound(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    try {
      identityOk = await this.verifier.verifyScopeSig(
        mreq.fromScope,
        mreq.id,
        mreq.toScope,
        sigHeader,
    ...
    if (!identityOk) {
      const denied = toMeshResponse(
        mreq.id,
        new CapabilityError("scope-sig invalid", ErrorCode.IDENTITY_SIG_INVALID),
      );
    ...
    try {
      response = await this.handler(mreq, {
        authenticatedScope: mreq.fromScope,
    ...
    return runWithRetry(
      async (): Promise<AttemptResult> => {
        ...
        try {
          ...
          if (r.statusCode === 200) {
            try {
              return { kind: "response", resp: decodeResponse(new Uint8Array(buf)) };
            } catch (e) {
              return { kind: "response", resp: toMeshResponse(req.id, e) };
            }
          }
          if (r.statusCode === 403) {
            try {
              return { kind: "fatal", resp: decodeResponse(new Uint8Array(buf)) };
            } catch {
              return {
                kind: "fatal",
                resp: {
                  id: req.id,
                  status: "denied",
                  error: { code: "E_CAP_INVALID", message: "denied" },
                },
              };
            }
          }
          ...
          if (r.statusCode === 409) {
            try {
              ...
              return { kind: "response", resp: { id: req.id, status: "ok" } };
            }
          }
          if (r.statusCode === 429) {
          ...
          return {
            ...
            resp: {
              id: req.id,
              status: "error",
              error: { code: "E_TRANSPORT", message: `http ${r.statusCode}` },
            },
          };
        } catch (e) {
