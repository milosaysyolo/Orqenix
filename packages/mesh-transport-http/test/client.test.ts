...
describe('HttpMeshTransport (client)', () => {
  it('maps 200 to ok', async () => {
    const s = new HttpMeshTransport({ localScopeId: 'scp_b3_B' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    s.onRequest(async (r) => ({ id: r.id, status: 'ok', payload: new Uint8Array([42]) }));
    await s.start();
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    await c.start();
    const resp = await c.send({ kind: 'http', baseUrl: `http://127.0.0.1:${s.port()}` }, mkReq(s.port()));
  ...
  it('maps network failure to timeout after retries', async () => {
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 1, baseDelayMs: 1 });
    await c.start();
  ...
  it('maps 403 to denied', async () => {
    ...
    const s = http.createServer((_req, res) => {
      ...
      res.end(Buffer.from([0x83, 0xa2, 0x69, 0x64, 0xa1, 0x78, 0xa6, 0x73, 0x74, 0x61, 0x74, 0x75, 0x73, 0xa6, 0x64, 0x65, 0x6e, 0x69, 0x65, 0x64]));
    });
    await new Promise<void>((resolve) => s.listen(0, '127.0.0.1', () => resolve()));
    const addr = s.address() as { port: number };
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 0 });
    await c.start();
    const resp = await c.send({ kind: 'http', baseUrl: `http://127.0.0.1:${addr.port}` }, mkReq(addr.port));
  ...
  it('maps 408 to timeout', async () => {
    ...
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 1, baseDelayMs: 1 });
    await c.start();
    const resp = await c.send({ kind: 'http', baseUrl: `http://127.0.0.1:${addr.port}` }, mkReq(addr.port, 'id408'));
    expect(resp.status).toBe('timeout');
    await c.stop();
    await new Promise<void>((resolve) => s.close(() => resolve()));
  });
  ...
  it('maps 200 with bad body to fallback', async () => {
    ...
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 0 });
    await c.start();
    const resp = await c.send({ kind: 'http', baseUrl: `http://127.0.0.1:${addr.port}` }, mkReq(addr.port, 'idBad200'));
    expect(resp.status).toBe('error');
    await c.stop();
    await new Promise<void>((resolve) => s.close(() => resolve()));
  });
  ...
  it('maps unknown status to error', async () => {
    ...
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 0 });
    await c.start();
    const resp = await c.send({ kind: 'http', baseUrl: `http://127.0.0.1:${addr.port}` }, mkReq(addr.port, 'id502'));
    expect(resp.status).toBe('error');
    await c.stop();
    await new Promise<void>((resolve) => s.close(() => resolve()));
  });
  ...
  it('maps 429 with retry-after to retry', async () => {
    ...
    await new Promise<void>((resolve) => s.listen(0, '127.0.0.1', () => resolve()));
    const addr = s.address() as { port: number };
    const c = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 2, baseDelayMs: 1 });
    await c.start();
    const resp = await c.send({ kind: 'http', baseUrl: `http://127.0.0.1:${addr.port}` }, mkReq(addr.port));
    expect(resp.status).toBe('ok');
