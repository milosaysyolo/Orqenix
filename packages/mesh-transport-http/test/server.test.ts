...
import { encodeRequest, decodeResponse, type CapabilityToken, type MeshRequest, type ScopeId } from '@orqenix/mesh-transport-core';
...
describe('HttpMeshTransport (server)', () => {
  ...
  it('serves a valid POST and returns ok', async () => {
    const s = new HttpMeshTransport({ localScopeId: 'scp_b3_B' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    s.onRequest(async (r) => ({ id: r.id, status: 'ok', payload: new Uint8Array([...r.payload, 0xff]) }));
    await s.start();
  ...
  it('dedup returns cached on duplicate id (409)', async () => {
    const s = new HttpMeshTransport({ localScopeId: 'scp_b3_B' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    let calls = 0;
    s.onRequest(async (r) => { calls++; return { id: r.id, status: 'ok' }; });
    await s.start();
    const url = `http://127.0.0.1:${s.port()}/orqenix/mesh/v1/rpc`;
  ...
  it('handler throwing maps to error', async () => {
    const s = new HttpMeshTransport({ localScopeId: 'scp_b3_B' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    s.onRequest(async (r) => { throw new Error('handler oops'); });
    await s.start();
    const req = mkReq();
  ...
  it('identity verification failure maps to 403 denied', async () => {
    const s = new HttpMeshTransport({ localScopeId: 'scp_b3_B' as ScopeId, verifier: new NullVerifier() as any, sign: NoopSigner });
