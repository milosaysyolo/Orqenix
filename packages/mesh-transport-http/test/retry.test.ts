...
describe('runWithRetry', () => {
  it('retries on timeout up to maxRetries', async () => {
    ...
    const resp = await runWithRetry(
      ...
      { maxRetries: 2, baseDelayMs: 1, deadlineMs: Date.now() + 1000, rand: () => 0.5, sleep: async () => {} },
    );
  ...
  it('never retries denied', async () => {
    let calls = 0;
    const denied: MeshResponse = { id: 'x', status: 'denied', error: { code: 'E_CAP_MISSING', message: 'no cap' } };
    const resp = await runWithRetry(
      async (): Promise<AttemptResult> => {
        calls++;
  ...
  it('respects deadline', async () => {
    const resp = await runWithRetry(
      async (): Promise<AttemptResult> => ({ kind: 'timeout' }),
      { maxRetries: 5, baseDelayMs: 1, deadlineMs: Date.now() - 1, rand: () => 0.5, sleep: async () => {} },
    );
    expect(resp.status).toBe('timeout');
  });
  ...
  it('aborts early on signal', async () => {
    ...
    const resp = await runWithRetry(
      async (): Promise<AttemptResult> => ({ kind: 'timeout' }),
      { maxRetries: 5, baseDelayMs: 1000, deadlineMs: Date.now() + 10000, signal: ac.signal, rand: () => 0.5, sleep: async () => {} },
    );
    expect(resp.status).toBe('timeout');
  });
  ...
  it('stops retrying after ok response', async () => {
    ...
    const resp = await runWithRetry(
      ...
      { maxRetries: 5, baseDelayMs: 1, deadlineMs: Date.now() + 1000, rand: () => 0.5, sleep: async () => {} },
    );
    expect(calls).toBe(1);
