...
describe('G37 gate: HTTP Mesh Transport', () => {
  ...
  it('C2 envelope canonical round-trip', async () => {
    ...
    server.onRequest(async (r) => ({ id: r.id, status: 'ok', payload: r.payload }));
    await server.start();
    const client = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 2, baseDelayMs: 1 });
    await client.start();
    try {
      const req = mkReq('01HV0R6X3M8YQ9G7F2D5W1ZZ02');
      const resp = await client.send({ kind: 'http', baseUrl: `http://127.0.0.1:${server.port()}` }, req);
  ...
  it('C3 dedup -> single handler call', async () => {
    ...
    let handlerCalls = 0;
    server.onRequest(async (r) => { handlerCalls++; return { id: r.id, status: 'ok', payload: r.payload }; });
    await server.start();
    const client = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 2, baseDelayMs: 1 });
    await client.start();
    try {
  ...
  it('C5 forged scope-sig -> denied', async () => {
    ...
    const client = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 0, baseDelayMs: 1 });
    await client.start();
    try {
  ...
  it('C8 dedup LRU bound enforced', async () => {
    ...
    await small.start();
    const c2 = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner, maxRetries: 0, baseDelayMs: 1 });
    await c2.start();
    try {
