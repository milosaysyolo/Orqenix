...
describe('Part 2 smoke: HTTP transport end-to-end', () => {
  it('A -> B over HTTP returns ok with payload, dedup single handler call, clean teardown', async () => {
    const server = new HttpMeshTransport({ localScopeId: 'scp_b3_B' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    let calls = 0;
    ...
    const client = new HttpMeshTransport({ localScopeId: 'scp_b3_A' as ScopeId, verifier: new NoopIdentityVerifier(), sign: NoopSigner });
    await client.start();
