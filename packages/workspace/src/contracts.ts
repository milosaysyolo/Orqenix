...
export class WorkspaceNotFoundError extends OrqenixError {
  constructor(id: string) {
    super(`workspace not found: ${id}`, "WORKSPACE_NOT_FOUND");
  }
}
export class WorkspaceAlreadyExistsError extends OrqenixError {
  constructor(id: string) {
    super(`workspace already exists: ${id}`, "WORKSPACE_EXISTS");
  }
}
export class MembershipNotFoundError extends OrqenixError {
  constructor(ws: string, scope: string) {
    super(`membership not found: ${ws}/${scope}`, "MEMBERSHIP_NOT_FOUND");
  }
}
export class MembershipAlreadyExistsError extends OrqenixError {
  constructor(ws: string, scope: string) {
    super(`membership already exists: ${ws}/${scope}`, "MEMBERSHIP_EXISTS");
  }
}
export class OwnerRemovalError extends OrqenixError {
  constructor() {
    super("cannot remove the owner from their own workspace", "OWNER_REMOVAL");
  }
}
