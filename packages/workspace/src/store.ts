...
export class WorkspaceStore {
  ...
  get(id: WorkspaceId): Workspace {
    const row = this.conn.prepare<WorkspaceRow>(`SELECT * FROM workspaces WHERE id = ?`).get(id) as
      | WorkspaceRow
      | undefined;
  ...
  removeMember(workspaceId: WorkspaceId, scopeId: string): boolean {
    ...
    const r = this.conn
      .prepare(`DELETE FROM workspace_memberships WHERE workspace_id = ? AND scope_id = ?`)
  ...
  changeRole(workspaceId: WorkspaceId, scopeId: string, newRole: MembershipRole): Membership {
    ...
    this.conn
      .prepare(`UPDATE workspace_memberships SET role = ? WHERE workspace_id = ? AND scope_id = ?`)
  ...
  transferOwnership(workspaceId: WorkspaceId, newOwnerScopeId: string): Workspace {
    ...
    const newOwnerMember = this.conn
      .prepare<MembershipRow>(
        `SELECT * FROM workspace_memberships WHERE workspace_id = ? AND scope_id = ?`,
      )
    ...
    this.conn.transaction(() => {
      ...
      this.conn
        .prepare(`UPDATE workspaces SET owner_scope_id = ? WHERE id = ?`)
        .run(newOwnerScopeId, workspaceId);
    });
