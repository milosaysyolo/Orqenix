...
export const ScopeYamlSchema = z.object({
  ...
  name: z.string().min(1).max(128).regex(/^[a-z0-9]+(-[a-z0-9]+)*$/, 'name must be kebab-case'),
  publicKey: z.string().regex(/^[A-Za-z0-9+/]{43}=$/, 'publicKey must be base64-encoded 32 bytes (44 chars)'),
...
export class InvalidScopeIdError extends OrqenixError {
  ...
  constructor(value: unknown) {
    super(`invalid scope id: ${typeof value === 'string' ? value : typeof value}`, 'INVALID_SCOPE_ID');
  }
}
...
export class KeyPairMismatchError extends OrqenixError {
  ...
  constructor() {
    super('public key in scope.yaml does not match the private key in identity.key', 'KEYPAIR_MISMATCH');
  }
}
