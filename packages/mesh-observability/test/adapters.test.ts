...
describe("observability adapters", () => {
  ...
  it("rpc.denied carries the errorCode and increments counter with status=denied", () => {
    ...
    const snap = metrics.snapshot();
    const denied = snap.counters.find(
      (c) => c.name === METRIC_NAMES.RPC_TOTAL && c.labels.includes("status=denied"),
    );
  ...
  it("peer connect/disconnect updates the peers gauge", () => {
    ...
    onPeerConnect(
      { logger, metrics },
      { scopeId: "scp_b3_A" as unknown as ScopeId, transport: "libp2p", peerCount: 3 },
    );
    onPeerDisconnect(
      { logger, metrics },
      { scopeId: "scp_b3_A" as unknown as ScopeId, transport: "libp2p", peerCount: 2 },
    );
    const snap = metrics.snapshot();
  ...
  it("failover increments failover_total with from/to labels", () => {
    ...
    onFailover(
      { logger, metrics },
      { scopeId: "scp_b3_A" as unknown as ScopeId, from: "libp2p", to: "http" },
    );
    const snap = metrics.snapshot();
  ...
  it("circuit open/halfopen/close sets gauge to 2/1/0", () => {
    ...
    onCircuitOpen(
      { logger, metrics },
      { scopeId: "scp_b3_A" as unknown as ScopeId, transport: "libp2p" },
    );
    expect(
      metrics.snapshot().gauges.find((g) => g.name === METRIC_NAMES.CIRCUIT_STATE)?.value,
    ).toBe(2);
    onCircuitHalfOpen(
      { logger, metrics },
      { scopeId: "scp_b3_A" as unknown as ScopeId, transport: "libp2p" },
