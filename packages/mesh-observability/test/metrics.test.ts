...
describe("MeshMetrics", () => {
  ...
  it("counters add up with labels", () => {
    ...
    const httpOk = snap.counters.find(
      (c) => c.name === METRIC_NAMES.RPC_TOTAL && c.labels.includes("transport=http"),
    );
    const libOk = snap.counters.find(
      (c) => c.name === METRIC_NAMES.RPC_TOTAL && c.labels.includes("transport=libp2p"),
    );
