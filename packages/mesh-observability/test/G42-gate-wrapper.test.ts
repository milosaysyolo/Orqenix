...
import {
  validateLogEvent, CANONICAL_EVENTS, type MeshLogEvent,
  METRIC_NAMES, MeshMetrics,
  bufferSink, MeshLogger,
  onRpcIn, onRpcOut, buildOutgoingTraceContext, traceIdOf, containsLeak, redact,
} from '../src/index.js';
...
describe('G42 gate', () => {
  it('C1 log schema validates over captured stream', () => {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      onRpcOut({ logger, metrics }, { scopeId: 'scp_b3_A' as unknown as ScopeId, transport: 'http' }, req, resp, i % 25);
    }
  ...
  it('C3 traceparent propagates across >=2 hops', () => {
    ...
    const distinctSpans = new Set([parent.traceparent, hop1.traceparent, hop2.traceparent]).size === 3;
