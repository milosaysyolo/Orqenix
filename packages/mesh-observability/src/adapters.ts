...
export function onPeerDisconnect(
  ...
  ctx: CommonCtx & { peerCount: number },
): void {
  h.logger.emit({
    level: "info",
...
export function onRpcOut(
  ...
  h.metrics.incCounter(METRIC_NAMES.RPC_TOTAL, 1, {
    transport: ctx.transport,
    status: resp.status,
  });
  h.metrics.observeHistogram(METRIC_NAMES.RPC_DURATION_MS, durationMs, {
    transport: ctx.transport,
  });
...
export function onCircuitOpen(
  h: ObservabilityHooks,
  ctx: { scopeId: ScopeId; transport: string },
): void {
  h.logger.emit({
    level: "warn",
    event: "circuit.open",
...
export function onCircuitHalfOpen(
  h: ObservabilityHooks,
  ctx: { scopeId: ScopeId; transport: string },
): void {
  h.logger.emit({
    level: "info",
    event: "circuit.halfopen",
    scopeId: ctx.scopeId,
    transport: ctx.transport,
  });
  h.metrics.setGauge(METRIC_NAMES.CIRCUIT_STATE, 1, { transport: ctx.transport });
}
export function onCircuitClose(
  h: ObservabilityHooks,
  ctx: { scopeId: ScopeId; transport: string },
): void {
  h.logger.emit({
