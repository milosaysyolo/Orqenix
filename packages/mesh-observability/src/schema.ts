...
export function validateLogEvent(value: unknown): string[] | null {
  ...
  if (v.peerId !== undefined && typeof v.peerId !== 'string') errs.push('peerId must be string when present');
  if (v.requestId !== undefined && typeof v.requestId !== 'string') errs.push('requestId must be string when present');
  if (v.method !== undefined && typeof v.method !== 'string') errs.push('method must be string when present');
  ...
  if (v.status !== undefined && !['ok', 'denied', 'error', 'timeout'].includes(v.status as string)) {
