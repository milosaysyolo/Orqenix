...
// @orqenix/memory-engine , Hierarchy query (parallel 3-step, INV-12)
//
// Queries session + branch + project levels in PARALLEL via Promise.all,
...
export class HierarchyQuery {
   ...
   *
   * Per INV-12: ALL levels query simultaneously; NO short-circuit when one
   * level returns hits. Performance bounded by slowest level, not sum.
   */
  async query(input: HierarchyQueryInput): Promise<HierarchyQueryResult> {
    const startMs = Date.now();
    const boosts: LevelBoosts = {
      ...DEFAULT_LEVEL_BOOSTS,
      ...input.levelBoosts,
    };
    ...
    const clusterThreshold =
      input.clusterCosineThreshold ?? DEFAULT_CLUSTER_COSINE;

    // ── Determine which levels to query (based on active links) ───────────
    const levelsToQuery: Array<'session' | 'branch' | 'project'> = [];
    const queryPromises: Array<Promise<{ level: 'session' | 'branch' | 'project'; results: ReturnType<HybridSearch['search']> }>> = [];
    ...
    // ── INV-12: parallel execution, NO short-circuit ─────────────────────
    const perLevel = await Promise.all(queryPromises);

    // ── Merge with weighted level boost + protection boost ───────────────
    ...
    for (const { level, results } of perLevel) {
      ...
      for (const result of results) {
        let finalScore = result.rawScore * levelBoost;

        // Protection boost for subagent returns (always surface)
    ...
    // ── Dedup by content hash (entries promoted across levels) ───────────
    ...
    // ── Cluster near-duplicates by embedding similarity ──────────────────
    const sorted = Array.from(byHash.values()).sort(
    ...
    // ── Anti-noise threshold (protection entries bypass threshold) ───────
    const filtered = clusters.filter(
      (r) =>
        r.finalScore >= minRelevance ||
        r.entry.protection_flags?.kind === 'subagent_return'
    );
