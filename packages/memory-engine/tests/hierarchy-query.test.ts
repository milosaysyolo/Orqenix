...
describe("HierarchyQuery (INV-12 parallel 3-step)", () => {
  ...
  it("boosts subagent returns by x10", async () => {
    ...
    db.prepare(
      ...
      "hsub1",
      "T1",
      "billing subagent return result",
      null,
      PROJECT,
      BRANCH,
      SESSION,
  ...
  it("applies anti-noise threshold", async () => {
    ...
    for (const r of result.results) {
      expect(r.finalScore >= 0.99 || r.entry.protection_flags?.kind === "subagent_return").toBe(
        true,
      );
    }
  });
