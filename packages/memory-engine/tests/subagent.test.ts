...
describe('SubagentHarnessManager (ADR-E-002 + Anti-pattern 36)', () => {
  ...
  it('invokes a subagent and returns the result', async () => {
    const invocation = await manager.invoke({
      parentSessionId: PARENT_SESSION, branchId: BRANCH, projectId: PROJECT,
      harness: makeHarness(),
  ...
  it('blocks sub-subagent spawning (Anti-pattern 36)', async () => {
    await expect(
      manager.invoke({
        parentSessionId: PARENT_SESSION, branchId: BRANCH, projectId: PROJECT,
        harness: makeHarness({
          constraints: { maxSteps: 5, maxWallTimeSec: 90, allowedTools: ['invoke_subagent'], forbiddenTools: [] },
        }),
        runner: async () => makeReturn(),
      })
  ...
  it('enforces maxSteps limit', async () => {
    await expect(
      manager.invoke({
        parentSessionId: PARENT_SESSION, branchId: BRANCH, projectId: PROJECT,
        harness: makeHarness({ constraints: { maxSteps: 2, maxWallTimeSec: 90, allowedTools: [], forbiddenTools: [] } }),
  ...
  it('enforces wall-time timeout', async () => {
    await expect(
      manager.invoke({
        parentSessionId: PARENT_SESSION, branchId: BRANCH, projectId: PROJECT,
        harness: makeHarness({ constraints: { maxSteps: 5, maxWallTimeSec: 0.05, allowedTools: [], forbiddenTools: [] } }),
...
describe('ReturnAbsorber (T1+T2, INV-13)', () => {
  ...
  it('absorbs return into T1 and T2', () => {
    const result = absorber.absorb({
      ret: makeReturn(), subagentSessionId: 'subX',
      subagentKind: 'test-runner', parentSessionId: PARENT_SESSION,
      branchId: BRANCH, projectId: PROJECT,
    });
...
describe('Compress guards (INV-13)', () => {
  function makeEntry(flags: MemoryEntry['protection_flags']): MemoryEntry {
    return {
      id: 'e1', hash: 'h1', kb: 'chat', tier: 'T1', content: 'x', embedding: null,
      project_id: PROJECT, branch_id: BRANCH, session_id: PARENT_SESSION, memory_level: 'session',
      protection_flags: flags,
      cloned_from_branch_id: null, promoted_from_session_id: null, promoted_from_branch_id: null,
  ...
  it('never compresses subagent_return entries', () => {
    const flags = makeSubagentReturnFlags({ subagentSessionId: 'subX', subagentKind: 'test', parentSessionId: PARENT_SESSION });
  ...
  it('validateProtectionFlags rejects incomplete subagent_return', () => {
    const result = validateProtectionFlags({
      kind: 'subagent_return', immutable: true, never_compress: false, never_move_tier: true,
    });
