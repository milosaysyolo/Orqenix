...
describe("BranchStore (ADR-E-003 deep-copy)", () => {
  ...
  it("isolation: writes to child do not affect parent", () => {
    const result = store.createBranch({
      parentBranchId: MAIN,
      newBranchName: "feature/iso",
      projectId: PROJECT,
    });

    const parentCountBefore = (
      db.prepare("SELECT COUNT(*) AS c FROM decision_entries WHERE branch_id = ?").get(MAIN) as {
        c: number;
      }
    ).c;
  ...
  it("respects cloneTiers=t1_only", () => {
    const result = store.createBranch({
      parentBranchId: MAIN,
      newBranchName: "feature/t1",
      projectId: PROJECT,
      cloneTiers: "t1_only",
    });
    const t4Count = result.cellSnapshot.T4
      ? Object.values(result.cellSnapshot.T4).reduce((s, n) => s + (n as number), 0)
      : 0;
