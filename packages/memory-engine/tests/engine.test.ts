...
describe('MemoryEngine (E2E keystone)', () => {
  ...
  it('writes a memory entry + audits it', async () => {
    const entry = await engine.write({
      kb: 'decision', content: 'Use Stripe for billing',
      branch_id: MAIN, session_id: '01J3X8H9SESS', memory_level: 'session',
    });
  ...
  it('queries across hierarchy', async () => {
    await engine.write({ kb: 'decision', content: 'billing via Stripe', branch_id: MAIN, memory_level: 'branch' });
  ...
  it('creates a branch via deep-copy + audits', async () => {
    await engine.write({ kb: 'decision', content: 'main decision', branch_id: MAIN, memory_level: 'branch' });
    const result = await engine.createBranch({ parentBranchId: MAIN, newBranchName: 'feature/billing' });
  ...
  it('invokes subagent + absorbs return to T1+T2 + audits', async () => {
    const ret: SubagentReturn = { output: { passed: 100, failed: 0 }, outputMatchesSchema: true, wallTimeMs: 5000, stepsTaken: 2 };

    const absorbed = await engine.invokeSubagent({
      parentSessionId: '01J3X8H9PARENT', branchId: MAIN,
      harness: {
        systemPrompt: 'test-runner', scopedContext: { entryIds: [], rationale: 'tests' },
        goal: 'run tests',
        constraints: { maxSteps: 5, maxWallTimeSec: 90, allowedTools: ['run_shell'], forbiddenTools: [] },
  ...
  it('subagent return surfaces with x10 boost in subsequent query', async () => {
    await engine.write({
      kb: 'chat', content: 'discussion about billing',
      branch_id: MAIN, session_id: '01J3X8H9PARENT', memory_level: 'session',
    });

    await engine.invokeSubagent({
      parentSessionId: '01J3X8H9PARENT', branchId: MAIN,
      harness: {
        systemPrompt: 'analyzer', scopedContext: { entryIds: [], rationale: 'x' },
        goal: 'analyze billing',
        constraints: { maxSteps: 5, maxWallTimeSec: 90, allowedTools: [], forbiddenTools: [] },
        returnSchema: { type: 'object' }, subagentKind: 'analyzer',
      },
      runner: async () => ({
        output: 'billing analysis complete', outputMatchesSchema: true, wallTimeMs: 1000, stepsTaken: 1,
      }),
    });

    const result = await engine.query({
      query: 'billing', kbs: ['chat'],
      sessionId: '01J3X8H9PARENT', branchId: MAIN, limit: 10,
    });
  ...
  it('promotes entry session->branch + audits', async () => {
    const entry = await engine.write({
      kb: 'decision', content: 'session decision to promote',
      branch_id: MAIN, session_id: '01J3X8H9SESS', memory_level: 'session',
    });

    await engine.promote({
      entryId: entry.id, kb: 'decision', from: 'session', to: 'branch',
      fromSessionId: '01J3X8H9SESS', fromBranchId: MAIN, reason: 'critical decision',
    });
  ...
  it('fetchContent resolves entry (wires federation stub)', async () => {
    const entry = await engine.write({
      kb: 'decision', content: 'fetchable content', branch_id: MAIN, memory_level: 'branch',
    });
