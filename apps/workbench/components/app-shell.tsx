...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* SIDEBAR */}
      <aside className="w-[200px] shrink-0 border-r border-[var(--line)] bg-[var(--paper)] px-2 py-4">
        <Link href="/" className="flex items-center gap-2 px-3">
          <span className="block h-3 w-3 rotate-45 rounded-[3px] bg-gradient-to-br from-[var(--rust)] to-[var(--plum)]" />
          <span className="font-mono text-sm font-extrabold tracking-wide">ORQENIX</span>
        </Link>
              ...
              {group.items.map((it) => {
                ...
                return (
                  <Link
                    ...
                    href={it.href}
                    className={
                      'flex items-center gap-2.5 border-l-2 px-3 py-1.5 font-mono text-[12px] transition-colors ' +
                      (active
                        ? 'border-[var(--rust)] bg-[color-mix(in_oklab,var(--rust)8%,transparent)] font-extrabold text-[var(--rust)]'
                        : 'border-transparent font-medium text-[var(--dim)] hover:text-[var(--ink)]')
                    }
                  >
                    <span className="w-3.5 text-center opacity-75">{it.glyph}</span>
                    {it.label}
                  </Link>
                );
              })}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-12 items-center gap-4 border-b border-[var(--line)] bg-[color-mix(in_oklab,var(--paper)94%,transparent)] px-5 backdrop-blur">
