    ...
    <div className="container mx-auto px-6 py-8 max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => router.push(`/marketplace/${encodeURIComponent(name)}/edit`)}>
            Update
          </Button>
          <Button variant="ghost" onClick={() => void handleDelete()}>Delete</Button>
        </div>
      </div>
      <Card>
        <CardHeader><CardTitle>Plugin detail</CardTitle></CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Full plugin detail (overview / permissions / conformance / stats) renders here.
            Data loaded via /api/marketplace?action=read&name={name}.
          </p>
        </CardContent>
      </Card>
    </div>
