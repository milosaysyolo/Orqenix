...
import { SettingsRegistry, type ModuleSettingsContract } from "@orqenix/settings-registry";
 ...
 */
export async function bootstrapSettings(registry: SettingsRegistry): Promise<void> {
...
export const ALL_MODULE_CONTRACTS: ModuleSettingsContract[] = [
  // ─── Phase 2: Memory Engine Core ─────────────────────────────────────
  {
    ...
    provenance: {
      phase: 2,
      crVersion: "v7.0",
      rationale: "Memory injection strategy B (medium) default locked in Phase 2",
    },
    hotReloadable: true,
