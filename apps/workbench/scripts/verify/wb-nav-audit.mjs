...
try {
  ...
  if (!/<nav\b/.test(shell)) { console.error('\u274c no <nav> in app-shell'); problems++; }
  if (linkSource < 2) { console.error('\u274c only ' + linkSource + ' <Link> in source (need \u22652 to produce 19 rendered links)'); problems++; }
  if (/\/ flex items|{it\.href}={/.test(shell)) { console.error('\u274c mangled Link leftovers (/ flex items or {it.href}={)'); problems++; }
  const navItemCount = (shell.match(/item\.href|{ href:/g) ?? []).length;
  if (problems === 0) console.log('\u2713 app-shell has <nav> + ' + linkSource + ' source <Link>s mapping ' + navItemCount + ' nav items');
} catch (e) { console.error('\u274c app-shell.tsx: ' + e.message); problems++; }
...
try {
  ...
  if (!/<AppShell>/.test(layout) || !/from '@\/components\/app-shell'/.test(layout)) {
    console.error('\u274c root layout does not wrap <AppShell>'); problems++;
  } else console.log('\u2713 root layout wraps <AppShell>');
} catch (e) { console.error('\u274c app/layout.tsx: ' + e.message); problems++; }
...
if (missing.length) { console.error('\u274c missing pages: ' + missing.join(', ')); problems++; }
