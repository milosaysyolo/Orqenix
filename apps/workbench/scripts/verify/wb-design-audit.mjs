...
function grep(pattern, label) {
  try {
    ...
    if (out.trim()) { console.error(`\u274c ${label}:`); out.trim().split('\n').forEach((l) => console.error('   ' + l)); problems++; }
  } catch { /* no match = good */ }
}
...
try {
  ...
  if (/--background:\s*\d|--foreground:\s*\d|hsl\(var\(--/.test(css)) { console.error('\u274c globals.css still has shadcn HSL tokens'); problems++; }
  if (/@apply\s+border-border|@layer\s+base[^}]*\*\s*{[^}]*@apply/.test(css)) { console.error('\u274c globals.css has shadcn @apply reset'); problems++; }
...
try {
  ...
  if (/Shell|<aside|sidebar/i.test(groupLayout)) { console.error('\u274c (workbench)/layout.tsx renders a shell \u2014 must be passthrough'); problems++; }
