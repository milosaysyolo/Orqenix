...
function spawnNode(dir: string): ChildProcess {
  ...
  const p = spawn(NODE_BIN, [BIN, "start", "--config", dir], {
    env,
...
async function waitForLine(proc: ChildProcess, regex: RegExp, timeoutMs = 15_000): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const t = setTimeout(() => {
      proc.stdout?.off("data", onData);
      reject(
        new Error(`Timed out waiting for ${regex}. Captured: ${lines.join("").slice(0, 1000)}`),
      );
    }, timeoutMs);
    proc.once("exit", (code) => {
      clearTimeout(t);
...
describe("Phase 6 e2e", () => {
  RUN(
    "starts a node, prints listening addresses, stops cleanly",
    async () => {
  ...
  RUN(
    "two nodes can both start independently",
    async () => {
