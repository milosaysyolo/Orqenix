...
describe('identity loader', () => {
  it('loads scope.yaml + private.pem and exposes 32-byte seed + CryptoKey objects', async () => {
    ...
    try {
      ...
      await writeFile(join(dir, 'identity', 'scope.yaml'), `
...
`,
      ...
      const id = await loadLocalIdentity(join(dir, 'identity', 'scope.yaml'), join(dir, 'identity', 'private.pem'));
  ...
  it('rejects missing PEM block', async () => {
    ...
    try {
      await mkdir(join(dir, 'identity'), { recursive: true });
      await writeFile(join(dir, 'identity', 'scope.yaml'), `scope_id: x\npublic_key_b64: "${toBase64(new Uint8Array(32))}"`);
      await writeFile(join(dir, 'identity', 'private.pem'), 'not a pem');
      await expect(
        loadLocalIdentity(join(dir, 'identity', 'scope.yaml'), join(dir, 'identity', 'private.pem')),
      ).rejects.toThrow();
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
