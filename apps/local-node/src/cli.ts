...
async function runStart(configDir: string): Promise<void> {
  ...
  let bootstrap;
  try { bootstrap = await loadBootstrapFile(bootstrapPath); } catch { /* optional */ }
  let addressBook = new AddressBook();
  try { addressBook = await loadPeersYaml(peersPath); } catch { /* optional */ }
  ...
  const runtime: LocalNodeRuntime = await startLocalNode({
    identity, config, bootstrap, addressBook, handler,
  });
...
export async function runCli(argv: string[]): Promise<void> {
  ...
  switch (args.cmd) {
    case 'start':   return runStart(args.configDir);
    case 'status':  return runStatus(args.configDir);
    case 'verify':  return runVerify();
    case 'version': runVersion(); return;
    case 'help':
    default:        runHelp(); return;
  }
}
