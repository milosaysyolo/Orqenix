...
describe('CROSS-PHASE SEAMS', () => {
  ...
  afterEach(async () => { try { engine?.close(); } catch {} engine = undefined; });
  ...
  it('SEAM Phase8→Phase3+7: write goes through storage AND audits', async () => {
    ...
    const entry = await engine.write({ kb: 'decision', content: 'seam test', branch_id: MAIN, memory_level: 'branch' });
  ...
  it('SEAM Federation→Engine: federation query routes to engine store', async () => {
    ...
    try {
      ...
      await writeFile(projectsYaml, stringifyYaml({
        projects: [{ id: PROJECT, name: 'self', path: tmpDir, registered_at: new Date().toISOString(), cross_project_sharing_enabled: true }],
      }));
      const fed = new FederationEngine({
        currentProjectId: PROJECT as never, userId: 'milo',
        projectsYamlPath: projectsYaml, approvalsYamlPath: approvalsYaml,
      });
  ...
  it('SEAM Settings→Engine: settings resolve through hierarchy', async () => {
    ...
    await registry.register({
      moduleId: '@orqenix/memory-engine', version: '0.8.0', settingsSchema: {},
      defaults: { 'hierarchy.level_boost.session': 1.5 },
      provenance: { phase: 8, crVersion: 'v8.0', rationale: 'seam' },
      hotReloadable: true, hierarchyOverride: 'all',
    });
  ...
  it('SEAM Observer→Detection→DB: pattern detection over observations', async () => {
    const obsEngine = await MemoryEngine.open(':memory:', { projectId: PROJECT, bootstrapBaseTables: true });
    ...
    for (let i = 0; i < 6; i++) {
      ...
      insertStmt.run(ulid(), PROJECT, null, `s${i}`, null, t1.toISOString(), null, 'agent', 'x', 'file_edit', JSON.stringify({ f: i }), null, null, null, 0, null);
      insertStmt.run(ulid(), PROJECT, null, `s${i}`, null, t2.toISOString(), null, 'agent', 'x', 'test_run', JSON.stringify({}), 'success', 5000, null, 0, null);
    }
