...
describe('PHASE 8 — Full Stack', () => {
  ...
  it('3-level hierarchy query works (parallel, INV-12)', async () => {
    ...
    await engine.write({ kb: 'decision', content: 'session billing', branch_id: MAIN, session_id: 's1', memory_level: 'session' });
    await engine.write({ kb: 'decision', content: 'branch billing', branch_id: MAIN, memory_level: 'branch' });
    const result = await engine.query({ query: 'billing', sessionId: 's1', branchId: MAIN, limit: 10 });
  ...
  it('subagent invoke + absorb to T1+T2 works (ADR-E-002)', async () => {
    ...
    const absorbed = await engine.invokeSubagent({
      parentSessionId: 's1', branchId: MAIN,
      harness: {
        systemPrompt: 'test-runner', scopedContext: { entryIds: [], rationale: 'x' },
        goal: 'run tests', constraints: { maxSteps: 5, maxWallTimeSec: 90, allowedTools: [], forbiddenTools: [] },
        returnSchema: { type: 'object' }, subagentKind: 'test-runner',
      },
      runner: async () => ({ output: { passed: 10 }, outputMatchesSchema: true, wallTimeMs: 1000, stepsTaken: 2 }),
    });
  ...
  it('normalization round-trip fidelity works (INV-15)', async () => {
    const engine2 = new NormalizationEngine({
      inputAdapters: ALL_INPUT_ADAPTERS, outputAdapters: ALL_OUTPUT_ADAPTERS,
    });
    expect(engine2.listInputAdapters().length).toBe(14);
    expect(engine2.listOutputAdapters().length).toBe(8);
    const pkg = JSON.stringify({
      name: '@a/b', version: '1.0.0', license: 'Apache-2.0', main: './p.js',
      keywords: ['orqenix-plugin'],
      orqenixPlugin: { manifestVersion: '1.0', kind: 'skill', compatibility: { orqenix: '~0.8.0' }, permissions: [], external_agent_compat: [], sandboxMode: 'separate_process' },
    }, null, 2);
  ...
  it('MCP server exposes 10 tools + 9 resources + 6 prompts', async () => {
    ...
    const server = new OrqenixMcpServer({
      ...
      transport: 'stdio', clientId: 'merge-test',
    });
